...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('manifesta_nfse_tomadas')) {
            Schema::create('manifesta_nfse_tomadas', function (Blueprint $table) {
                $table->unsignedBigInteger('id')->autoIncrement();
                $table->unsignedInteger('empresa_id');
                $table->unsignedInteger('filial_id')->nullable();
                $table->unsignedInteger('usuario_id')->nullable();
                $table->string('chave', 100)->nullable();
                $table->string('nsu', 20)->nullable()->default('0');
                $table->string('numero_nota', 15)->nullable();
                $table->dateTime('data_emissao')->nullable();
                $table->string('prestador_nome', 255)->nullable();
                $table->string('prestador_cnpj_cpf', 18)->nullable();
                $table->decimal('valor_servico', 15, 2)->nullable()->default(0);
                $table->decimal('valor_liquido', 15, 2)->nullable()->default(0);
                $table->unsignedInteger('compra_servico_id')->nullable();
                $table->boolean('fatura_salva')->default(false);
                $table->timestamps();

                $table->index('empresa_id', 'idx_manifesta_nfse_tomadas_empresa_id');
                $table->index('filial_id', 'idx_manifesta_nfse_tomadas_filial_id');
                $table->index('usuario_id', 'idx_manifesta_nfse_tomadas_usuario_id');
                $table->unique('chave', 'uk_manifesta_nfse_tomadas_chave');
                $table->index(['empresa_id', 'filial_id', 'nsu'], 'idx_manifesta_nfse_tomadas_nsu_empresa');
            });
        } else {
            $this->ensureBaseTenantColumns('manifesta_nfse_tomadas');
        }

        $this->addForeignIfMissing('manifesta_nfse_tomadas', 'manifesta_nfse_tomadas_empresa_id_foreign', 'empresa_id', 'empresas', 'cascade');
        $this->addForeignIfMissing('manifesta_nfse_tomadas', 'manifesta_nfse_tomadas_filial_id_foreign', 'filial_id', 'filials', 'set null');
        $this->addForeignIfMissing('manifesta_nfse_tomadas', 'manifesta_nfse_tomadas_usuario_id_foreign', 'usuario_id', 'usuarios', 'set null');
    }

    public function down()
    {
        if (!Schema::hasTable('manifesta_nfse_tomadas')) { return; }
        $this->dropForeignIfExists('manifesta_nfse_tomadas', 'manifesta_nfse_tomadas_usuario_id_foreign');
        $this->dropForeignIfExists('manifesta_nfse_tomadas', 'manifesta_nfse_tomadas_filial_id_foreign');
        $this->dropForeignIfExists('manifesta_nfse_tomadas', 'manifesta_nfse_tomadas_empresa_id_foreign');
        Schema::dropIfExists('manifesta_nfse_tomadas');
    }
