	...
	<div class="card-body">
		<div class="content d-flex flex-column flex-column-fluid" id="kt_content" >
			...
			<div class="col-lg-12" id="content">
				<!--begin::Portlet-->
				<input type="hidden" id="filial" value="{{$orcamento->filial_id}}">
				<h3 class="card-title">Orçamento código: <strong>{{$orcamento->id}}</strong></h3>
				@if(empresaComFilial())
				<h3 class="card-titl">Localização: 
					<strong class="text-danger">{{$orcamento->filial ? $orcamento->filial->descricao : 'Matriz'}}</strong>
				</h3>
				@endif
							...
							<div class="kt-section__body">
								...
								<div class="row">
									<div class="col-lg-6 col-md-6 col-sm-6 col-12">
										<h5>Cliente: <strong class="">{{$orcamento->cliente->razao_social}}</strong>
											<button data-toggle="modal" data-target="#modal-clientes" class="btn btn-sm btn-info">
												<i class="la la-edit"></i>
												Alterar cliente
											</button>
										</h5>
										<h5>CNPJ: <strong class="">{{$orcamento->cliente->cpf_cnpj}}</strong></h5>
										<h5>Data: <strong class="">{{ \Carbon\Carbon::parse($orcamento->created_at)->format('d/m/Y H:i:s')}}</strong></h5>
										<h5>Valor Integral: <strong class="">{{ number_format($orcamento->valor_total, $casasDecimais, ',', '.') }}</strong></h5>
										<h5>Cidade: <strong class="">{{ $orcamento->cliente->cidade->nome }} ({{ $orcamento->cliente->cidade->uf }})</strong></h5>
										<h5>Dias restantes para o vencimento: <strong class="">{{ $diasParaVencimento }}</strong></h5>
										...
										@if($orcamento->ecommerce)
										<h5 class="text-danger">*orçamento realizado pelo ecommerce</h5>
										@endif

										@if($orcamento->vendedor_id)
										<h5>Vendedor: <strong class="text-info">{{ $orcamento->vendedor_setado->funcionario->nome }}</strong></h5>
										@endif

										@if($orcamento->cliente->cpf_cnpj == "")
										<p class="text-danger">
											<i class="la la-warning text-danger"></i>
											Este cliente possui dados de cadastros pendentes, preencha os dados para transformar o orçamento em venda!
										</p>
										<a class="btn btn-warning" href="/clientes/edit/{{$orcamento->cliente->id}}">
											<i class="la la-edit"></i>
											Editar Cliente
										</a>
										@endif
								...
								<div class="form-group col-lg-12 col-md-12 col-sm-12 col-12">
									...
									<div class="">
										<div class="input-group">
											<input value="{{$orcamento->observacao}}" type="text" name="observacao" class="form-control" id="observacao"/>
										</div>
									</div>
								</div>
							</div>
							...
							<table class="datatable-table" style="max-width: 100%; overflow: scroll">
								...
								<tbody id="body" class="datatable-body">
									...
									<tr class="datatable-row">
										...
										<td class="datatable-cell"><span class="codigo" style="width: 100px;">{{ number_format($i->quantidade, $casasDecimaisQtd) }}</span>
										</td>
										...
										<td class="datatable-cell">
											<span class="codigo" style="width: 100px;">
												...
												<a href="/orcamentoVenda/deleteItem/{{$i->id}}" class="btn btn-sm btn-danger">
...
<div class="modal fade" id="modal-clientes" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
	<div class="modal-dialog modal-xl" role="document">
		<div class="modal-content">
			...
			<form method="post" action="/orcamentoVenda/alterarCliente">
				@csrf
				<div class="modal-body">
					<div class="row">
						...
						<div class="form-group validated col-12">
							...
							<div class="">
								<select class="custom-select form-control select2-custom" name="cliente_id">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-light-danger font-weight-bold" data-dismiss="modal">Fechar</button>
					<button type="submit" class="btn btn-light-success font-weight-bold">Alterar</button>
