	...
	public function handle($request, Closure $next){
                ...
                if($value){
                        ...
                        if(!$usuario){
                                ...
                                session()->flash('mensagem_login', 'Usuário não encontrado, faça login novamente');
                                return redirect('/login');
                        }

                        if($usuario->acessos && sizeof($usuario->acessos) > 0){
                                $ult = $usuario->acessos[sizeof($usuario->acessos)-1];
                                $ult->updated_at = date('Y-m-d H:i:s');
                                $ult->save();
                        }

                        if($usuario->empresa && !$usuario->empresa->status && $value['super'] == 0){
