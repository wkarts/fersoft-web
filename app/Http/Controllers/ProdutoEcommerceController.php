...
class ProdutoEcommerceController extends Controller
{
	...
	public function new(){
		...
		if(sizeof($categorias) == 0){
			session()->flash("mensagem_alerta", "Cadastre uma categoria para o ecommerce!");
	...
	public function save(Request $request){
		...
		if($request->produto){
			...
			if(!$request->grade){
				...
				$request->merge([ 
					'sub_categoria_id' => $request->sub_categoria_id ?? 0,
					'categoria_ecommerce_id' => $request->categoria_id
				]);
				...
				$res = $produtoGrade->salvarDoEcommerce($request, $nomeImagem, null, $arr);

				session()->flash("mensagem_sucesso", "Produto(s) cadastrado com sucesso!");
		...
		$request->merge([ 'sub_categoria_id' => $request->sub_categoria_id ?? 0]);
		$request->merge([ 'cep' => $request->cep ?? '']);
    ...
    public function update(Request $request){
    	...
    	$resp->sub_categoria_id = $request->sub_categoria_id ?? 0;
