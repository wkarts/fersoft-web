...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('conciliacao_regras')) {
            Schema::create('conciliacao_regras', function (Blueprint $table) {
                ...
                $table->string('tipo_regra', 50)->default('contem');
                $table->unsignedBigInteger('fornecedor_id')->nullable();
                $table->unsignedBigInteger('cliente_id')->nullable();
                $table->unsignedBigInteger('categoria_id')->nullable();
                $table->timestamps();
                $table->index('empresa_id', 'idx_conciliacao_regras_empresa_id');
                $table->index('filial_id', 'idx_conciliacao_regras_filial_id');
                $table->index('usuario_id', 'idx_conciliacao_regras_usuario_id');
                $table->index('palavra_chave', 'idx_conciliacao_regras_palavra_chave');
                $table->index('fornecedor_id', 'idx_conciliacao_regras_fornecedor_id');
                $table->index('cliente_id', 'idx_conciliacao_regras_cliente_id');
            });
        } else {
            $this->ensureBaseTenantColumns('conciliacao_regras');
        }

        $this->addForeignIfMissing('conciliacao_regras', 'conciliacao_regras_empresa_id_foreign', 'empresa_id', 'empresas', 'cascade');
        $this->addForeignIfMissing('conciliacao_regras', 'conciliacao_regras_filial_id_foreign', 'filial_id', 'filials', 'cascade');
        $this->addForeignIfMissing('conciliacao_regras', 'conciliacao_regras_usuario_id_foreign', 'usuario_id', 'usuarios', 'cascade');
    }

    public function down()
    {
        if (!Schema::hasTable('conciliacao_regras')) { return; }
        $this->dropForeignIfExists('conciliacao_regras', 'conciliacao_regras_usuario_id_foreign');
        $this->dropForeignIfExists('conciliacao_regras', 'conciliacao_regras_filial_id_foreign');
        $this->dropForeignIfExists('conciliacao_regras', 'conciliacao_regras_empresa_id_foreign');
