...
<div class="card card-custom gutter-b">
    <div class="card-body">
        ...
        <div class="@if(env('ANIMACAO')) animate__animated @endif animate__backInRight" id="kt_user_profile_aside">
            <form method="get" action="/contasPagar/filtro">
                <div class="row align-items-center">
                    ...
                    <div class="form-group col-lg-2">
                        <label class="col-form-label font-weight-bold">Conta Bancária</label>
                        ...
                        <tr class="datatable-row">
                            ...
                            <td class="datatable-cell">
                                <span style="width: 160px;">
                                  ...
                                  {{-- Botão de Ver Detalhes (Fica FORA do if, então aparece sempre!) --}}
                                  <a href="/contasPagar/detalhes/{{$c->id}}" class="btn btn-primary btn-sm btn-icon" title="Ver Detalhes">
                                          <i class="la la-eye"></i>
                                      </a>
                            ...
                            <td class="datatable-cell">
                                <span style="width: 180px;">
                                    <span class="d-block font-weight-bold">{{ $c->fornecedor->razao_social ?? '--' }}</span>
                                    @if($c->veiculo)
                                        <span class="label label-inline label-light-primary font-weight-bold mt-1" style="font-size: 10px;">
                                            <i class="la la-truck mr-1"></i> {{ $c->veiculo->placa }}
                                        </span>
                                    @endif
                                </span>
                            </td>
        ...
        </div> </div> </div> {{-- (GRUPO) MODAIS: Janelas que abrem por cima da tela (Ex: Vincular Veículo) --}}
@foreach($contas as $c)
        ...
        <div class="modal-content">
            <form method="post" action="{{ route('contasPagar.setVeiculo', [$c->id]) }}">
                ...
                <div class="modal-body">
                    <select name="veiculo_id" class="form-control select2-veiculo" id="select_veiculo_{{$c->id}}">
                        <option value="">Nenhum</option>
                        @foreach($veiculos as $v)
                            <option @if($c->veiculo_id == $v->id) selected @endif value="{{$v->id}}">{{$v->placa}}</option>
                        @endforeach
                    </select>
                </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        // Inicializa o Select2 sempre que um modal for aberto
        $('[id^=modal_veiculo_]').on('shown.bs.modal', function () {
            $(this).find('.select2-veiculo').select2({
                dropdownParent: $(this),
                width: '100%' 
            });
        });
