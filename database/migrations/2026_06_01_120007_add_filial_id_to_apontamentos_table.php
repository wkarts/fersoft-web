...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('apontamentos')) { return; }

        Schema::table('apontamentos', function (Blueprint $table) {
            if (!Schema::hasColumn('apontamentos', 'filial_id')) {
                $table->unsignedInteger('filial_id')->nullable()->after('produto_id');
            }
        });

        $this->addIndexIfMissing('apontamentos', 'idx_apontamentos_filial_id', ['filial_id']);
    }

    public function down()
    {
        if (!Schema::hasTable('apontamentos')) { return; }
        $this->dropIndexIfExists('apontamentos', 'idx_apontamentos_filial_id');
        $this->dropColumnIfExists('apontamentos', 'filial_id');
    }
