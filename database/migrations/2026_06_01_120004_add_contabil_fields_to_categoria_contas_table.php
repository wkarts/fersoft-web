...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('categoria_contas')) { return; }

        Schema::table('categoria_contas', function (Blueprint $table) {
            if (!Schema::hasColumn('categoria_contas', 'conta_contabil_despesa_id')) {
                $table->unsignedInteger('conta_contabil_despesa_id')->nullable()->after('filial_id');
            }
            if (!Schema::hasColumn('categoria_contas', 'conta_contabil_provisao_id')) {
                $table->unsignedInteger('conta_contabil_provisao_id')->nullable()->after('conta_contabil_despesa_id');
            }
            if (!Schema::hasColumn('categoria_contas', 'ignora_terceiro')) {
                $table->boolean('ignora_terceiro')->default(false)->after('conta_contabil_provisao_id');
            }
            if (!Schema::hasColumn('categoria_contas', 'gera_provisao')) {
                $table->boolean('gera_provisao')->default(false)->after('ignora_terceiro');
            }
        });

        $this->addIndexIfMissing('categoria_contas', 'idx_conta_contabil_despesa', ['conta_contabil_despesa_id']);
        $this->addIndexIfMissing('categoria_contas', 'idx_conta_contabil_provisao', ['conta_contabil_provisao_id']);
    }

    public function down()
    {
        if (!Schema::hasTable('categoria_contas')) { return; }
        $this->dropIndexIfExists('categoria_contas', 'idx_conta_contabil_provisao');
        $this->dropIndexIfExists('categoria_contas', 'idx_conta_contabil_despesa');
        $this->dropColumnIfExists('categoria_contas', 'gera_provisao');
        $this->dropColumnIfExists('categoria_contas', 'ignora_terceiro');
        $this->dropColumnIfExists('categoria_contas', 'conta_contabil_provisao_id');
        $this->dropColumnIfExists('categoria_contas', 'conta_contabil_despesa_id');
    }
