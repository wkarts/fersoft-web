...
{
    private const REFERENCE_COLUMNS = ['empresa_id' => ['empresas', 'id', 'cascade', 'restrict'], 'filial_id' => ['filials', 'id', 'set null', 'cascade'], 'plano_conta_id' => ['plano_contas', 'id', 'set null', 'cascade'], 'usuario_id' => ['usuarios', 'id', 'set null', 'cascade']];
    private const MIGRATION_NAME = '2026_09_18_120102_update_conta_empresas_table';
    private const TABLE_NAME = 'conta_empresas';

    public function up()
    {
        $this->runWithSchemaLock(function () {
            // Valida o plano desta tabela antes do primeiro DDL; não preenche dados por suposição.
            $this->ensureTableExists('conta_empresas');
            $this->preflightColumn('conta_empresas', 'conta_contabil_id', ['column_type' => 'int unsigned', 'nullable' => true, 'column_default' => null, 'extra' => '', 'character_set_name' => null, 'collation_name' => null, 'column_comment' => null]);

            $this->ensureColumn('conta_empresas', 'conta_contabil_id', function (Blueprint $table) {
                return $table->unsignedInteger('conta_contabil_id')->nullable()->default(null);
