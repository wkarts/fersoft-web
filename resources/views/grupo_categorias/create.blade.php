@php
    $title = $title ?? 'Novo Grupo de Categorias';
@endphp
        ...
        <div class="col-md-10">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">
                        <i class="fas fa-folder-plus me-2"></i> Novo Grupo de Categorias
                    </h5>
                ...
                <div class="card-body">
                    <form action="{{ url('/grupo-categorias/salvar') }}" method="POST">
                        @csrf

                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                <label for="nome" class="form-label fw-bold">Nome do Grupo <span class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('nome') is-invalid @enderror" id="nome" name="nome" value="{{ old('nome') }}" placeholder="Ex: Frotas, TI, Administrativo" required>
                                @error('nome')
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="descricao" name="descricao" value="{{ old('descricao') }}" placeholder="Finalidade deste agrupamento">
                            </div>
                        </div>
                            ...
                            @forelse($categorias as $cat)
                                <div class="col item-categoria">
                                    <div class="form-check card p-2 border rounded shadow-xs h-100">
                                        <input class="form-check-input ms-1 me-2" type="checkbox" name="categorias[]" value="{{ $cat->id }}" id="cat_{{ $cat->id }}">
