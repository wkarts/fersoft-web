...
{	
    ...
    public function gerarVenda($id){
    	...
    	return view('cotacao/ajustar_venda', compact('item'));
    }

    public function salvarVenda(Request $request, $id){
    	...
    	for($i=0; $i<sizeof($request->item_id); $i++){
    		...
    		$produto->percentual_lucro = __replace($request->percentual_lucro[$i]);
    		$produto->save();
    	}

    	return redirect('vendas/nova?cotacao_id='.$id);
    }
