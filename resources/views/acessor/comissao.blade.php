...
<div class="card card-custom gutter-b">
	...
	<div class="card-body">
		...
		<div class="" id="kt_user_profile_aside" style="margin-left: 10px; margin-right: 10px;">
			...
			<form method="get" action="/acessores/comissaoFiltro">
				<div class="row align-items-center">
					<input type="hidden" value="{{$item->id}}" name="acessor_id">
						...
						<div class="">
							<div class="input-group date">
								<select class="custom-select form-control" id="estado" name="status">
									...
									<option @if(isset($status) && $status == '1') selected @endif value="1">PAGO</option>
									<option @if(isset($status) && $status == '0') selected @endif value="0">PENDENTE</option>
								</select>
							</div>
						</div>
					</div>
									...
									<tr class="datatable-row">
										...
										<td class="datatable-cell">
											<span class="codigo" style="width: 100px;">
												@if($e->venda)
												{{ number_format($e->venda->valor_total, 2, ',', '.') }}
												@else
												--
												@endif
											</span>
										...
										<?php 
										if($e->venda){
											$somaVendas += $e->venda->valor_total;
										}
										?>
										...
										<td class="datatable-cell"><span class="codigo" style="width: 150px;">
											...
											<a class="btn btn-danger" title="REMOVER" onclick='swal("Atenção!", "Deseja remover este registro?", "warning").then((sim) => {if(sim){ location.href="/acessores/comissaoDelete/{{$e->id}}" }else{return false} })' href="#!">
...
<script type="text/javascript">
	...
	function pagar(){
		...
		.then((sim) => {
			if(sim)
				location.href=path+'acessores/pagarComissao?arr='+arr;
		})
