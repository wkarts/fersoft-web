...
class DFeController extends Controller
{
    ...
    private function getFornecedorXML($xml){
        ...
        $cnpj = isset($xml->NFe->infNFe->emit->CNPJ) ? (string) $xml->NFe->infNFe->emit->CNPJ : '';
        $cpf = isset($xml->NFe->infNFe->emit->CPF) ? (string) $xml->NFe->infNFe->emit->CPF : '';
        ...
        $fornecedor = [
            ...
            'nomeFantasia' => isset($xml->NFe->infNFe->emit->xFant) ? (string) $xml->NFe->infNFe->emit->xFant : (string) $xml->NFe->infNFe->emit->xNome,
            ...
            'cep' => isset($xml->NFe->infNFe->emit->enderEmit->CEP) ? (string) $xml->NFe->infNFe->emit->enderEmit->CEP : '',
            'fone' => isset($xml->NFe->infNFe->emit->enderEmit->fone) ? (string) $xml->NFe->infNFe->emit->enderEmit->fone : '',
            'ie' => isset($xml->NFe->infNFe->emit->IE) ? (string) $xml->NFe->infNFe->emit->IE : '',
            'cidade_id' => $cidade ? $cidade->id : 1 // Evita erro se a cidade não existir
        ];

        $fornecedorEncontrado = $this->verificaFornecedor($documento);

        if($fornecedorEncontrado){
            $fornecedor['novo_cadastrado'] = false;
            $fornecedor['id'] = $fornecedorEncontrado->id; // Pega o ID
        }else{
            $fornecedor['novo_cadastrado'] = true;
            $fornecedor['id'] = $this->cadastrarFornecedor($fornecedor); // Cria e pega o ID
        }
    ...
    private function getItensDaNFe($xml){
        ...
        foreach($xml->NFe->infNFe->det as $item) {
            ...
            // Se começou com 5 (Dentro do Estado), muda pra 1. Se 6 (Fora), muda pra 2.
            ...
            if(isset($item->imposto->ICMS)){
                ...
                $icms_key = array_key_first($icms_arr); // Ex: ICMS00, ICMS40, ICMSSN101
                if($icms_key){
                    if(isset($item->imposto->ICMS->$icms_key->CST)) $cst_icms = (string) $item->imposto->ICMS->$icms_key->CST;
                    elseif(isset($item->imposto->ICMS->$icms_key->CSOSN)) $cst_icms = (string) $item->imposto->ICMS->$icms_key->CSOSN;
                }
            }
            ...
            // --- REGRA FISCAL DE/PARA: PIS E COFINS ---
            // Se a nota veio com saída tributada (01 a 05), a entrada é com crédito (50)
            ...
            $itemArr = [
                ...
                'xProd' => $nomeProduto,
                'NCM' => $item->prod->NCM,
                'CFOP' => $cfop_entrada,         // Manda o CFOP já traduzido
                'CST_ICMS' => $cst_icms,         // Manda o ICMS
                'CST_PIS' => $cst_pis_entrada,   // Manda o PIS traduzido
                'CST_COFINS' => $cst_cofins_entrada, // Manda o COFINS traduzido
                'CEST' => $item->prod->CEST,
                'uCom' => $item->prod->uCom,
    ...
    private function getFaturaDaNFe($xml){
        ...
        // PLANO B: Se a nota NÃO TEM duplicatas (Ex: Sucata à vista), gera uma parcela integral
        else {
            // Pega o valor total da nota
            $vNF = number_format((double) $xml->NFe->infNFe->total->ICMSTot->vNF, 2, ",", ".");

            // Extrai a data de emissão para usar como vencimento
    ...
    public function salvarFatura(Request $request){
        ...
        foreach($fatura as $fat){
            // Busca o valor tentando os dois nomes possíveis (valor_parcela ou valor)
            $valorBruto = $fat->valor_parcela ?? $fat->valor ?? '0';

            // Limpa a pontuação para o formato do banco (Ex: 1.250,50 -> 1250.50)
            ...
            ContaPagar::create([
                ...
                'valor_pago' => 0,
                // Ajuste aqui também para garantir que o número da parcela e NF apareçam
    ...
    public function salvar(Request $request) {
        try {
            ...
            $chave = trim($request->chave);

            // 1. Busca a compra pela CHAVE
            $compraExistente = Compra::where('empresa_id', $this->empresa_id)
                ->where('chave', $chave)
                ->first();

            if ($compraExistente) {
                // 2. Busca fatura vinculada ao ID real dessa compra
                $financeiro = ContaPagar::where('empresa_id', $this->empresa_id)
                    ->where('compra_id', $compraExistente->id)
                    ->first();

                if (!$financeiro) {
                    // Se a nota está lá mas o financeiro NÃO, gera as faturas
                    $this->gerarFatura($compraExistente, $request);
                    \DB::commit();
                    session()->flash('mensagem_sucesso', 'Financeiro gerado com sucesso para a nota existente!');
                    return redirect()->back();
                }
            ...
            // --- AQUI ESTAVA O ERRO: Removi o bloco que dava erro de duplicidade antes da hora ---
            ...
            $result = Compra::create([
                ...
                'usuario_id' => get_id_user() ?? auth()->user()->id,
                'nf' => $request->nNf,
            ...
            // 5. GRAVA OS ITENS
            foreach($itens as $i){
                $qCom = is_object($i->qCom) ? get_object_vars($i->qCom)[0] : $i->qCom;
                $vUnCom = is_object($i->vUnCom) ? get_object_vars($i->vUnCom)[0] : $i->vUnCom;
                ...
                ItemCompra::create([
                    ...
                    'cfop_entrada' => $request->cfop_entrada[$i->codigo] ?? $i->CFOP,
                    'cst_icms' => $request->cst_icms_entrada[$i->codigo] ?? null,
                    'cst_pis' => $request->cst_pis_entrada[$i->codigo] ?? null,
                    'cst_cofins' => $request->cst_cofins_entrada[$i->codigo] ?? null,
                ]);

                // Atualiza Estoque
                if(method_exists($this, 'atualizarEstoque')){
                    $this->atualizarEstoque($i, $result->id);
                }
            }

            // 6. GERA A FATURA (Seja nota nova ou se clicou em salvar fatura)
            $this->gerarFatura($result, $request);

            \DB::commit();
            session()->flash('mensagem_sucesso', 'Nota e Fatura salvas com sucesso!');
            return redirect()->back();
