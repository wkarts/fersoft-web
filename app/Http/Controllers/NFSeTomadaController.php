...
class NFSeTomadaController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        // Mantém os campos limpos para a Query
        $numero_nota = $request->input('numero_nota');
        $fornecedor = $request->input('fornecedor');
        $filial_filtro = $request->input('filial_id'); // 🌟 Adicionado filtro por filial na tela se desejar
        $data_inicial = $request->input('data_inicial') ?: date('01/m/Y');
        $data_final = $request->input('data_final') ?: date('t/m/Y');

        $query = ManifestaNfseTomada::query()
            ->where('manifesta_nfse_tomadas.empresa_id', $emp_id)
            ->where('manifesta_nfse_tomadas.numero_nota', '!=', '0') // Remove números 0
            ->where('manifesta_nfse_tomadas.valor_servico', '>', 0);

        // 🌟 FILTRO: Número da Nota (Garante remoção de zeros à esquerda se necessário)
        if (!empty($numero_nota)) {
            $query->where(function($q) use ($numero_nota) {
                $q->where('manifesta_nfse_tomadas.numero_nota', $numero_nota)
                    ->orWhere('manifesta_nfse_tomadas.numero_nota', 'LIKE', "%".ltrim($numero_nota, '0')."%");
            });
        }

        // 🌟 FILTRO: Fornecedor / Prestador
        // 🌟 FILTRO: Fornecedor / Prestador (Ajustado para ler os dois campos)
        $nome_prestador = $request->input('nome_prestador');
        $cnpj_prestador = $request->input('cnpj_prestador');

        if (!empty($nome_prestador) || !empty($cnpj_prestador)) {
            $query->where(function($q) use ($nome_prestador, $cnpj_prestador) {

                // Filtra por nome se preenchido
                if (!empty($nome_prestador)) {
                    $q->where('manifesta_nfse_tomadas.prestador_nome', 'LIKE', "%{$nome_prestador}%");
                }

                // Filtra por CNPJ se preenchido
                if (!empty($cnpj_prestador)) {
                    $cnpjLimpo = preg_replace('/[^0-9]/', '', $cnpj_prestador);
                    $q->orWhere('manifesta_nfse_tomadas.prestador_cnpj_cpf', 'LIKE', "%{$cnpjLimpo}%");
                }
            });
        }

        // 🌟 FILTRO: Filial específica (Caso queira filtrar na listagem)
        if (!empty($filial_filtro)) {
            if ($filial_filtro == 'matriz') {
                $query->whereNull('manifesta_nfse_tomadas.filial_id');
            } else {
                $query->where('manifesta_nfse_tomadas.filial_id', $filial_filtro);
            }
        }
        ...
        if ($data_inicial && $data_final) {
            try {
                ...
                // Evita quebra se a data vier em formato inválido
            }
        }
        ...
        $docs = $query->select([
            ...
            ->paginate(20)
            ->appends($request->all()); // 🌟 CRUCIAL: Mantém os filtros ativos ao mudar de página!

        $filiais = \App\Models\Filial::where('empresa_id', $emp_id)->get();

        return view('nfse_tomadas.index', compact('docs', 'filiais', 'data_inicial', 'data_final', 'numero_nota', 'fornecedor', 'filial_filtro'))
            ...
            while ($consultasRealizadas < $limiteDeConsultasPorClique) {
                ...
                if (isset($resultado['LoteDFe']) && is_array($resultado['LoteDFe']) && count($resultado['LoteDFe']) > 0) {
                    foreach ($resultado['LoteDFe'] as $doc) {
                        ...
                        // 🔥 CORREÇÃO: Prioriza data do XML, depois do Documento da Receita, e só então erro/debug
                        $dataEmissao = null;
                        if (isset($inf->dhEmi)) {
                            ...
                            // Caso não ache, não preencha com data atual (pode gerar erros fiscais). Deixe null ou trate como erro.
                            $dataEmissao = null;
                        }

                        // 🔥 NOVO: Identificação de Cancelamento
                        $situacao = 'NORMAL';
                        // Verifica se existe tag de cancelamento (padrão varia, mas geralmente é cStat 101 ou tag Sit)
                        if (isset($inf->cStat) && (string)$inf->cStat == '101') {
                            $situacao = 'CANCELADA';
                        }
    ...
    public function detalhesLançamento($id)
    {
        ...
        $infos = [
            'vProd' => $nota->valor_servico,
            'vDesc' => 0, // Se houver desconto na nota, ajuste aqui
            'vNF'   => $nota->valor_liquido > 0 ? $nota->valor_liquido : $nota->valor_servico
        ];
    ...
    public function salvarImportacaoPainel(Request $request, $id)
    {
        ...
        try {
            ...
            // ========================================================
            // 3. GERAÇÃO DAS FATURAS (LENDO O JSON DO RATEIO DA VIEW)
            // ========================================================
            $descricaoServico = !empty($nota->prestador_nome) ? "Prestação de Serviço - " . $nota->prestador_nome : "Serviço Tomado";

            // Decodifica o JSON gerado pelo JavaScript na view
            $faturas = json_decode($request->fatura_json, true);

            if (!empty($faturas) && is_array($faturas)) {

                // Se o frontend mandou as faturas prontas (rateio, à vista ou prazo manual)
                foreach ($faturas as $index => $fat) {

                    // Converte a data de DD/MM/YYYY para YYYY-MM-DD
                    $dataVencimento = \Carbon\Carbon::createFromFormat('d/m/Y', $fat['vencimento'])->format('Y-m-d');

                    // Converte o valor de R$ padrão BR para formato de banco (Float)
                    $valorFatura = str_replace('.', '', $fat['valor_parcela']);
                    $valorFatura = (float) str_replace(',', '.', $valorFatura);
                    ...
                    \App\Models\ContaPagar::create([
                        ...
                        'data_emissao'       => $nota->data_emissao,
                        'valor_integral'     => $valorFatura,
                        'valor_original'     => $valorFatura,
                        'status'             => false,
                        'referencia'         => "NFS-e " . $nota->numero_nota . " - " . substr($descricaoServico, 0, 50) . " (" . $fat['numero'] . ")",
                        'categoria_id'       => $request->categoria_conta_id,
                        'empresa_id'         => $emp_id,
                        'filial_id'          => $nota->filial_id,
                        'veiculo_id'         => !empty($fat['veiculo_id']) ? $fat['veiculo_id'] : $request->veiculo_id, // Pega o veículo do rateio ou o geral
                        'numero_nota_fiscal' => ltrim($nota->numero_nota, '0'),
                        'usuario_id'         => $usuario_id,
                        ...($index == 0 ? $retencoes : []) // Aplica as retenções de impostos apenas na primeira parcela
                    ]);
                }

            } else {

                // FALLBACK: Caso o JSON falhe ou venha vazio, mantém a lógica de parcelas padrão
                $qtdParcelas = (int)$request->quantidade_parcelas > 0 ? (int)$request->quantidade_parcelas : 1;
                $valorParcela = round($vLiquido / $qtdParcelas, 2);
                $somaAcumulada = 0;
                $prazo = $request->prazo_pagamento ?? 30; // Usa 30 dias se não vier nada

                for ($i = 1; $i <= $qtdParcelas; $i++) {
                    $vencimento = date('Y-m-d', strtotime($nota->data_emissao . " + " . ($prazo * $i) . " days"));
    ...
    public function imprimirEspelho($id)
    {
        try {
            ...
            // O XML já está na pasta certinha como você mencionou
            $caminhoXml = public_path('xml_servico/' . $nota->chave . '.xml');
            $xmlString = null;
            ...
            if (file_exists($caminhoXml)) {
                ...
                $xmlString = base64_decode($nota->xml_base64);
            ...
            if ($xmlString) {
                ...
                // 2. Extrair Dados do Endereço do Prestador (Fornecedor)
                $enderecoPrestador = 'Não informado no XML';
                if (isset($xml->infNFSe->emit->enderNac)) {
                    $enderecoPrestador = (string)$xml->infNFSe->emit->enderNac->xLgr . ', ' . (string)($xml->infNFSe->emit->enderNac->nro ?? 'S/N') . ' - ' . (string)($xml->infNFSe->emit->enderNac->xBairro ?? '');
                } elseif (isset($xml->infNFSe->prestador->endereco)) {
                    $enderecoPrestador = (string)$xml->infNFSe->prestador->endereco->endereco . ', ' . (string)($xml->infNFSe->prestador->endereco->numero ?? 'S/N');
                }

                // 3. Extrair Dados do Tomador (Sua Empresa / O Cliente)
                $tomadorNode = null;

                if (isset($xml->infNFSe->DPS->infDPS->toma)) {
                    $tomadorNode = $xml->infNFSe->DPS->infDPS->toma;
                } elseif (isset($xml->DPS->infDPS->toma)) {
                    $tomadorNode = $xml->DPS->infDPS->toma;
                } elseif (isset($xml->infNFSe->toma)) {
                    $tomadorNode = $xml->infNFSe->toma;
                } elseif (isset($xml->Nfse->InfNfse->TomadorServico)) { // Prefeituras antigas
                    $tomadorNode = $xml->Nfse->InfNfse->TomadorServico;
                }

                $tomador = [
                    'nome' => 'Consumidor / Não Informado',
                    'cnpj_cpf' => '',
                    'endereco' => 'Não informado no XML'
                ];

                if ($tomadorNode) {
                    // Se for o Padrão Nacional (NFS-e Nacional usa a tag <xNome>)
                    if (isset($tomadorNode->xNome)) {
                        $tomador['nome'] = (string)$tomadorNode->xNome;
                        $tomador['cnpj_cpf'] = (string)($tomadorNode->CNPJ ?? $tomadorNode->CPF ?? '');

                        if (isset($tomadorNode->end)) {
                            $lgr = (string)($tomadorNode->end->xLgr ?? '');
                            $nro = (string)($tomadorNode->end->nro ?? 'S/N');
                            $bairro = (string)($tomadorNode->end->xBairro ?? '');

                            // 🔥 Pegando CEP e Código do Município
                            $cep = '';
                            $cMun = '';
                            if (isset($tomadorNode->end->endNac)) {
                                $cep = (string)($tomadorNode->end->endNac->CEP ?? '');
                                $cMun = (string)($tomadorNode->end->endNac->cMun ?? '');
                            }

                            // Formatando o CEP visualmente (00000-000)
                            if (strlen($cep) == 8) {
                                $cep = substr($cep, 0, 5) . '-' . substr($cep, 5, 3);
                            }

                            // 🔥 Buscando o nome do município no banco de dados
                            $nomeCidade = $cMun;
                            if (!empty($cMun)) {
                                try {
                                    // Tenta buscar na tabela 'cidades' do ERP.
                                    // OBS: Se a sua coluna de código IBGE tiver outro nome (ex: 'ibge'), mude abaixo:
                                    $cidadeDb = \Illuminate\Support\Facades\DB::table('cidades')
                                        ->where('codigo_ibge', $cMun)
                                        ->first();

                                    if ($cidadeDb) {
                                        // Ajuste 'nome' se a sua coluna de nome da cidade for diferente
                                        $nomeCidade = $cidadeDb->nome;
                                    }
                                } catch (\Exception $e) {
                                    // Se a tabela não existir, ignora o erro e mostra o código numérico no lugar
                                }
                            }

                            // Montando o endereço completo bonitinho para o Espelho
                            $enderecoCompleto = $lgr . ', ' . $nro;
                            if ($bairro) $enderecoCompleto .= ' - ' . $bairro;
                            if ($nomeCidade) $enderecoCompleto .= ' - ' . $nomeCidade;
                            if ($cep) $enderecoCompleto .= ' - CEP: ' . $cep;

                            $tomador['endereco'] = $enderecoCompleto;
                        }
                        elseif (isset($tomadorNode->enderNac)) {
                            $tomador['endereco'] = (string)($tomadorNode->enderNac->xLgr ?? '') . ', ' . (string)($tomadorNode->enderNac->nro ?? 'S/N');
                        }
                    }
                    // Se for o Padrão Antigo ABRASF (Usa a tag <RazaoSocial>)
                    elseif (isset($tomadorNode->RazaoSocial)) {
                        $tomador['nome'] = (string)$tomadorNode->RazaoSocial;
                        $tomador['cnpj_cpf'] = (string)($tomadorNode->IdentificacaoTomador->CnpjCpf->Cnpj ?? $tomadorNode->IdentificacaoTomador->CnpjCpf->Cpf ?? '');

                        if (isset($tomadorNode->Endereco)) {
                            $enderecoCompleto = (string)($tomadorNode->Endereco->Endereco ?? '') . ', ' . (string)($tomadorNode->Endereco->Numero ?? 'S/N');

                            $cep = (string)($tomadorNode->Endereco->Cep ?? '');
                            $cMun = (string)($tomadorNode->Endereco->CodigoMunicipio ?? '');
                            ...
                            if (!empty($cMun)) {
                                try {
                                    $cidadeDb = \Illuminate\Support\Facades\DB::table('cidades')->where('codigo_ibge', $cMun)->first();
                                    if ($cidadeDb) $nomeCidade = $cidadeDb->nome;
                                } catch (\Exception $e) {}
                            }
                ...
                // 4. Capturar o ISS Retido do XML
                $valorIssRetido = 0;
                $tpRetISSQN = $xml->infNFSe->DPS->infDPS->valores->trib->tribMun->tpRetISSQN ?? null;

                if ($tpRetISSQN == '2') { // 2 = Retido no Padrão Nacional
                    $valorIssRetido = (float)($xml->infNFSe->valores->vISSQN ?? 0);
                } else {
                    $issRetidoAbrasf = $xml->Nfse->InfNfse->Servico->Valores->IssRetido ?? $xml->infNFSe->servico->valores->issRetido ?? null;
                    if ((string)$issRetidoAbrasf == '1') {
                        $valorIssRetido = (float)($xml->Nfse->InfNfse->Servico->Valores->ValorIssRetido ?? $xml->Nfse->InfNfse->Servico->Valores->ValorIss ?? 0);
                    }
                }

                // ========================================================
                // CÁLCULO DAS RETENÇÕES FEDERAIS PARA O ESPELHO
                // ========================================================
                $vServico = $nota->valor_servico;
                $vLiquido = $nota->valor_liquido > 0 ? $nota->valor_liquido : $vServico;
                $vPis = 0; $vCofins = 0; $vCsll = 0; $vIr = 0;
                ...
                $retencoes = [
                    'iss_retido' => $valorIssRetido,
                    'pis' => $vPis,
                    'cofins' => $vCofins,
                    'csll' => $vCsll,
                    'irrf' => $vIr,
                    'liquido' => $vLiquido,
                    'total_retido' => $diffFederal + $valorIssRetido
                ];
            ...
            return view('nfse.visualizar', [
                ...
                'title' => 'DANFSE - Nota ' . ($nota->numero_nota ?? 'N/A'),
                'tomador' => $tomadorDefault,
                'retencoes' => $retencoesDefault,
                'endereco_prestador' => 'Não informado'
            ]);
    ...
    public function buscarPagamentosManuais()
    {
        try {
            ...
            // 1. Pega as notas importadas que AINDA NÃO foram lançadas pelo painel
            $notasPendentes = ManifestaNfseTomada::where('empresa_id', $emp_id)
                ->where('fatura_salva', 0)
                ->where('numero_nota', '!=', '0') // Ignora registros inválidos
                ->get();
            ...
            foreach ($notasPendentes as $nota) {
                ...
                // 🔥 CORREÇÃO 1: Faltava a variável do número da nota aqui!
                $numeroNotaLimpo = ltrim($nota->numero_nota, '0');

                // Limpa o CNPJ do prestador deixando só números
                $cnpjNota = preg_replace('/[^0-9]/', '', $nota->prestador_cnpj_cpf);

                // Tenta pegar da chave de acesso (padrão nacional)
                if (empty($cnpjNota) && strlen($nota->chave) == 50) {
                    $cnpjNota = substr($nota->chave, 9, 14);
                }

                // 🔥 NOVO TRUQUE: Tenta extrair o CPF/CNPJ de dentro do Nome do Prestador!
                if (empty($cnpjNota) && !empty($nota->prestador_nome)) {
                    // Pega todos os números soltos no nome e verifica se formam 11 ou 14 dígitos
                    $numerosNoNome = preg_replace('/[^0-9]/', '', $nota->prestador_nome);

                    if (strlen($numerosNoNome) == 11 || strlen($numerosNoNome) == 14) {
                        $cnpjNota = $numerosNoNome;
                    }
                }

                // Se ainda assim não tiver CNPJ de jeito nenhum, aí sim pula
                if (empty($cnpjNota)) {
                    continue;
                } // 🔥 CORREÇÃO 2: ESTA ERA A CHAVE QUE ESTAVA FALTANDO!

                // 2. Procura no banco se alguém digitou essa conta manualmente no Contas a Pagar
                $pagamentoManual = DB::table('conta_pagars')
                    ->join('fornecedors', 'fornecedors.id', '=', 'conta_pagars.fornecedor_id')
                    ->where('conta_pagars.empresa_id', $emp_id)
                    // Compara o número da nota (transformando o que o usuário digitou em número inteiro)
                    ->whereRaw("CAST(conta_pagars.numero_nota_fiscal AS UNSIGNED) = ?", [$numeroNotaLimpo])
                    // Compara o CNPJ do fornecedor limpando a pontuação do banco
                    ->whereRaw("REPLACE(REPLACE(REPLACE(fornecedors.cpf_cnpj, '.', ''), '/', ''), '-', '') = ?", [$cnpjNota])
                    ->first();

                // 3. Se achou um lançamento manual compatível, fazemos o casamento!
                if ($pagamentoManual) {

                    // Atualiza a nota dizendo que a fatura já existe, para ela sumir da tela de "Lançar"
                    $nota->prestador_cnpj_cpf = $pagamentoManual->cpf_cnpj;
                    $nota->fatura_salva = 1;
                    $nota->save();
            ...
            session()->flash('mensagem_sucesso', "Varredura concluída! Encontramos e baixamos $vinculosFeitos notas que haviam sido lançadas manualmente.");

        } catch (\Exception $e) {
            session()->flash('mensagem_erro', 'Erro ao tentar buscar lançamentos manuais: ' . $e->getMessage());
        }
