...
@endforeach
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group validated col-sm-4 col-lg-2">
                                                ...
                                                <div class="">
                                                    <input id="latitude" type="text" class="form-control" name="latitude" value="{{{ isset($cliente) ? $cliente->latitude : old('latitude') }}}">
                                                </div>
                                            </div>
                                            <div class="form-group validated col-sm-4 col-lg-2">
                                                <label class="col-form-label">Longitude</label>
                                                <div class="">
                                                    <input id="longitude" type="text" class="form-control" name="longitude" value="{{{ isset($cliente) ? $cliente->longitude : old('longitude') }}}">
                                                </div>
                                            </div>
                                            <div class="form-group validated col-sm-4 col-lg-3">
                                                <br><br>
                                                <button type="button" class="btn btn-info btn-sm" onclick="buscarCoordenadaManual()">
                                                    📍 Buscar no Mapa
                                                </button>
                                            </div>
                                            <div class="form-group validated col-sm-8 col-lg-3">
    ...
    @section('javascript')
        <script type="text/javascript">
            ...
            function buscarCoordenadaManual() {
                let rua = document.getElementById('rua').value;
                let numero = document.getElementById('numero').value;
                let bairro = document.getElementById('bairro').value;
                let cidadeSelect = document.getElementById('kt_select2_1');
                let cidadeNome = cidadeSelect.options[cidadeSelect.selectedIndex].text;

                if(!rua || cidadeSelect.value === "") {
                    swal("Atenção", "Preencha a rua e selecione a cidade primeiro!", "warning");
                    return;
                }

                let enderecoCompleto = `${rua}, ${numero}, ${bairro}, ${cidadeNome}, Brasil`;
                let url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(enderecoCompleto)}&limit=1`;

                fetch(url, { headers: { 'User-Agent': 'FersoftERP/1.0' } })
                    .then(response => response.json())
                    .then(data => {
                        if(data.length > 0) {
                            document.getElementById('latitude').value = data[0].lat;
                            document.getElementById('longitude').value = data[0].lon;
                            swal("Sucesso!", "Coordenadas encontradas e preenchidas!", "success");
                        } else {
                            swal("Ops", "Endereço não encontrado no mapa gratuito. Digite manualmente se possuir os dados do Traccar.", "error");
                        }
                    }).catch(err => {
                    swal("Erro", "Erro ao comunicar com o servidor de mapas.", "error");
                });
            }
        </script>
    @endsection
