                    ...
                    <table class="table table-hover table-bordered table-striped" style="font-size: 13px;">
                        ...
                        @foreach($notas as $n)
                            @php
                                ...
                                $isDevolvida = in_array($n['situacao'], ['DEVOLVIDA CLIENTE', 'REJEITADA CLIENTE']);
                                // Bloqueia a integração se estiver cancelada, se for devolução logística ou se já estiver integrada
                                $podeIntegrar = !$isCancelada && !$isDevolvida && !$n['bloqueia_integracao'] && !$n['integrado'];
                            @endphp
                            <tr>
                                ...
                                <td class="text-center align-middle">
                                    @if($n['situacao'] == 'DEVOLVIDA CLIENTE')
                                        <span class="badge badge-danger">Devolvida Cliente</span>
                                    @elseif($n['situacao'] == 'REJEITADA CLIENTE')
                                        <span class="badge badge-warning text-dark">Rejeitada Cliente</span>
                                    @elseif(in_array($n['situacao'], ['APROVADO', 'AUTORIZADO']))
                                    ...
                                    <div class="mt-1">
                                        @if($isDevolvida)
                                            <small class="text-danger font-weight-bold"><i class="fa fa-undo"></i> Recusada Cliente</small>
                                        @elseif($n['bloqueia_integracao'])
