...
function buscar(call){
	$.get(path+'controleCozinha/buscar', {tela: $('#tela').val()})
	.done((data) => {
		call(data)
	})
	.fail((err) => {
...
function criaDiv(comanda, nome, quantidade, data, item_id, adicionais, 
	...
	'<h3 style="margin-top: 20px; color: white">'+tipo+': <strong class="text-success">'+ (comanda != null ? comanda : '')+
	'</strong></h3></div>'+
