...
@section('content')
            ...
            <div class="@if(env('ANIMACAO')) animate__animated @endif animate__backInLeft">
                <div class="col-12">
                    <div class="row">
                        ...
                        <a href="#" class="btn btn-sm btn-secondary" id="btnAtualizarIbsCbs" role="button" title="Preencher IBS/CBS/IS a partir da TIPI">
                            <i class="fa fa-arrow-up"></i> Atualizar IBS/CBS/IS
                        </a>
                                            ...
                                            <div class="card card-custom gutter-b example example-compact">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <div class="kt-widget__info">
                                                        <span class="kt-widget__label">IBS/CBS:</span>
                                                        @php
                                                            $cstIbs = data_get($p, 'cst_ibs_cbs') ?? data_get($p, 'CST_IBS_CBS');
                                                            $classIbs = data_get($p, 'class_trib_ibs_cbs') ?? data_get($p, 'CLASS_TRIB_IBS_CBS');
                                                            $redIbs = data_get($p, 'reducao_ibs') ?? data_get($p, 'REDUCAO_IBS');
                                                            $redCbs = data_get($p, 'reducao_cbs') ?? data_get($p, 'REDUCAO_CBS');
                                                            $flagIs = data_get($p, 'flag_is') ?? data_get($p, 'FLAG_IS');
                                                            $cstIs = data_get($p, 'cst_is') ?? data_get($p, 'CST_IS');
                                                            $aliqIs = data_get($p, 'aliq_is') ?? data_get($p, 'ALIQ_IS');
                                                            $ibsOk = trim((string)$cstIbs) !== '' && trim((string)$classIbs) !== '' && !is_null($redIbs) && !is_null($redCbs) && trim((string)$flagIs) !== '' && !is_null($cstIs) && !is_null($aliqIs);
                                                        @endphp
                                                        @if($ibsOk)
                                                            <span class="badge badge-light-success" style="vertical-align: middle;">OK</span>
                                                        @else
                                                            <span class="badge badge-light-danger" style="vertical-align: middle;">Pendente</span>
                                                        @endif
                                                        <span class="kt-widget__data" style="font-size: 12px;">CST: {{ $cstIbs ?: '-' }} | CT: {{ $classIbs ?: '-' }}</span>
                                                    </div>
                                                    <div class="kt-widget__info mb-2">
    ...
    <!-- Modal Atualizar IBS/CBS/IS (TIPI) -->
    <div class="modal fade" id="modalAtualizarIbsCbs" tabindex="-1" role="dialog" aria-hidden="true" style="display:none">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Atualizar IBS/CBS/IS (TIPI)</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="alert alert-secondary" style="margin-bottom: 12px;">
                        Preenche IBS/CBS/IS dos produtos pelo NCM, usando os dados da importação TIPI.
                    </div>
                    <label class="kt-checkbox kt-checkbox--solid" style="margin:0;">
                        <input type="checkbox" id="chkOnlyMissingIbsCbs" checked> Atualizar somente faltantes (recomendado)
                        <span></span>
                    </label>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-secondary" data-dismiss="modal">Cancelar</button>
                    <button type="button" class="btn btn-primary" id="btnExecutarAtualizarIbsCbs">Executar</button>
                </div>
            </div>
        </div>
    </div>
...
@section('javascript')
    ...
    <script>
        (function(){
            const btnOpen = document.getElementById('btnAtualizarIbsCbs');
            const btnExec = document.getElementById('btnExecutarAtualizarIbsCbs');
            const chkOnly = document.getElementById('chkOnlyMissingIbsCbs');
            const token = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '{{ csrf_token() }}';

            if (btnOpen) {
                btnOpen.addEventListener('click', function(e){
                    e.preventDefault();
                    $('#modalAtualizarIbsCbs').modal('show');
                });
            }

            if (btnExec) {
                btnExec.addEventListener('click', runUpdate);
            }

            async function runUpdate(){
                try {
                    $('#modalAtualizarIbsCbs').modal('hide');

                    Swal.fire({
                        title: 'Atualizando IBS/CBS/IS...',
                        allowOutsideClick: false,
                        didOpen: () => Swal.showLoading()
                    });

                    const onlyMissing = chkOnly ? !!chkOnly.checked : true;

                    const resp = await fetch('/produtos/ibs-cbs/atualizar', {
                        method: 'POST',
                        headers: {
                    ...
                    if (!resp.ok || !data.ok) {
                        const msg = data.message || 'Falha ao atualizar IBS/CBS/IS.';
                        throw new Error(msg);
                    }

                    const r = data.result || {};

                    // URLs do relatório para USUÁRIO FINAL
                    const reportUrl      = data.report_url || null;
                    const csvUpdatedUrl  = data.csv_updated_url || null;
                    const csvNoTipiUrl   = data.csv_no_tipi_url || null;

                    // monta bloco de links (sem abrir várias janelas)
                    let links = '';
                    if (reportUrl) {
                        links += '<a href="' + reportUrl + '" target="_blank" rel="noopener" class="btn btn-sm btn-outline-primary mr-1 mb-1">Abrir Relatório</a>';
                    }
                    if (csvUpdatedUrl) {
                        links += '<a href="' + csvUpdatedUrl + '" target="_blank" rel="noopener" class="btn btn-sm btn-outline-secondary mr-1 mb-1">CSV Atualizados</a>';
                    }
                    if (csvNoTipiUrl) {
                        links += '<a href="' + csvNoTipiUrl + '" target="_blank" rel="noopener" class="btn btn-sm btn-outline-secondary mr-1 mb-1">CSV Sem TIPI</a>';
                    }

                    const html =
                        'Alvo com TIPI: <b>' + (r.total_targets ?? '-') + '</b><br>' +
                        'Atualizados: <b>' + (r.updated ?? '-') + '</b><br>' +
                        'Sem alteração: <b>' + (r.unchanged ?? '-') + '</b><br>' +
                        'Sem TIPI (NCM): <b>' + (r.no_tipi ?? '-') + '</b><br>' +
                        'Tempo: <b>' + (r.duration_ms ?? '-') + ' ms</b>' +
                        (links ? (
                            '<hr>' +
                            '<div style="margin-bottom:6px;"><b>Relatórios para consulta:</b></div>' +
                            '<div>' + links + '</div>' +
                            '<div style="margin-top:8px; font-size:12px; color:#6b7280;">' +
                            'Se o navegador bloquear pop-up, use os botões acima para abrir.' +
                            '</div>'
                        ) : '');

                    // tenta abrir automaticamente só o relatório (1 janela)
                    if (reportUrl) {
                        try {
                    ...
                    Swal.fire({ icon: 'success', title: 'Concluído', html: html })
