...
return new class extends Migration
{
    public function up()
    {
        ...
        Schema::table('filials', function (Blueprint $table) {
            if (!Schema::hasColumn('filials', 'codigo_prosoft')) {
                $table->string('codigo_prosoft', 4)->nullable()->after('updated_at');
            }
            if (!Schema::hasColumn('filials', 'conta_estoque')) {
                $table->unsignedInteger('conta_estoque')->nullable()->after('codigo_prosoft');
            }
        });

        $this->addIndexIfMissing('filials', 'idx_filials_codigo_prosoft', ['codigo_prosoft']);
        $this->addIndexIfMissing('filials', 'idx_filials_conta_estoque', ['conta_estoque']);
    }

    public function down()
    {
        if (!Schema::hasTable('filials')) { return; }
        $this->dropIndexIfExists('filials', 'idx_filials_conta_estoque');
        $this->dropIndexIfExists('filials', 'idx_filials_codigo_prosoft');
        $this->dropColumnIfExists('filials', 'conta_estoque');
        $this->dropColumnIfExists('filials', 'codigo_prosoft');
    }
