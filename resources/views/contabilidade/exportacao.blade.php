    ...
    <div class="d-flex flex-column flex-column-fluid" id="kt_content">
        <div class="card card-custom gutter-b">
            <div class="card-header">
                ...
                <div class="card-toolbar d-flex align-items-center">
                    ...
                    <button type="button" class="btn btn-primary mr-2" onclick="gerarTerceiros()">
                        <i class="fa fa-users"></i> Gerar Terceiros (.txt)
                    </button>
        ...
        <div class="card-body">
            ...
            <ul class="nav nav-tabs nav-tabs-line" role="tablist">
                @foreach(['pagar_prov'=>'Pagar (Prov)', 'pagar_baixa'=>'Pagar (Baixa)', 'receber_prov'=>'Receber (Prov)', 'receber_baixa'=>'Receber (Baixa)', 'manual'=>'Banco / Caixa', 'adiantamentos'=>'Adiantamentos'] as $id => $label)
                    <li class="nav-item"><a class="nav-link {{ $loop->first ? 'active' : '' }}" data-toggle="tab" href="#tab_{{$id}}">{{$label}}</a></li>
                @endforeach
            </ul>

            <div class="tab-content mt-5">
                @foreach(['pagar_prov', 'pagar_baixa', 'receber_prov', 'receber_baixa', 'manual', 'adiantamentos'] as $id)
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p>O sistema lê todos os seus lançamentos financeiros (pagamentos, recebimentos, adiantamentos e Banco/Caixa) e os classifica automaticamente para a contabilidade. O segredo dessa automação são as <strong>Categorias Financeiras</strong>. É a categoria que diz ao sistema qual conta contábil usar.</p>
                    <hr>
                    ...
                    <ul>
                        ...
                        <li><span class="badge badge-warning text-white">SEM PROVISÃO</span> Ocorreu uma baixa no banco, mas a conta a pagar/receber original está sem a conta contábil configurada.</li>
                    ...
                    <ul>
                        <li><strong>Prosoft (.txt):</strong> Gera o arquivo posicional no padrão exigido pelo sistema contábil Prosoft. Verifique sempre se selecionou a Filial correta no filtro.</li>
                        <li><strong>Gerar Terceiros (.txt):</strong> Gera o cadastro de todos os clientes e fornecedores para enviar ao contador.</li>
                        <li><strong>Excel (.csv):</strong> Gera uma planilha universal para conferência detalhada.</li>
                    </ul>
                </div>
    ...
    <script>
        ...
        function processarPrevia() {
            ...
            $.post("{{ route('contabilidade.exportacao.previa') }}", {
                ...
                ['pagar_prov', 'pagar_baixa', 'receber_prov', 'receber_baixa', 'manual', 'adiantamentos'].forEach(id => {
        ...
        function gerarTerceiros() {
            let filial = $('#filial_id').val();
            let url = "{{ route('contabilidade.exportacao.terceiros') }}";
