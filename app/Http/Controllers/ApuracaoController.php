...
class ApuracaoController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $lancamentos_diretos = DB::table('item_conta_empresas as i')->join('categoria_contas as c', 'i.categoria_id', '=', 'c.id')
            ->select('c.dre_grupo', 'c.nome as categoria_nome', 'i.tipo', DB::raw("SUM(i.valor) as total"))
            ->where('i.empresa_id', $this->empresa_id)->where('c.incluir_resultado', 1)
            ->whereNull('i.conta_receber_id')->whereNull('i.conta_pagar_id')
            ->whereMonth('i.data_pagamento', $mes)->whereYear('i.data_pagamento', $ano)
            ->groupBy('c.dre_grupo', 'c.nome', 'i.tipo')->get();
        ...
        foreach ($receitas_res as $r) {
            ...
            if(isset($dados[$grupo])) {
                $dados[$grupo] += (float)$r->total;
                if(!isset($detalhes[$grupo][$r->categoria_nome])){
                    $detalhes[$grupo][$r->categoria_nome] = (object)['categoria_nome' => $r->categoria_nome, 'total' => 0];
                }
                $detalhes[$grupo][$r->categoria_nome]->total += (float)$r->total;
            }
        }
        ...
        foreach ($despesas_res as $d) {
            ...
            if(isset($dados[$grupo])) {
                $dados[$grupo] -= (float)$d->total;
                if(!isset($detalhes[$grupo][$d->categoria_nome])){
                    $detalhes[$grupo][$d->categoria_nome] = (object)['categoria_nome' => $d->categoria_nome, 'total' => 0];
                }
                $detalhes[$grupo][$d->categoria_nome]->total += (float)$d->total; // Guardamos positivo para o number_format
            }
        }

        foreach ($lancamentos_diretos as $l) {
            $grupo = trim($l->dre_grupo);
            if(isset($dados[$grupo])) {
                $valorAjustado = ($l->tipo == 'entrada') ? (float)$l->total : (float)$l->total * -1;
                $dados[$grupo] += $valorAjustado;
                if(!isset($detalhes[$grupo][$l->categoria_nome])){
                    $detalhes[$grupo][$l->categoria_nome] = (object)['categoria_nome' => $l->categoria_nome, 'total' => 0];
                }
                $detalhes[$grupo][$l->categoria_nome]->total += ($l->tipo == 'entrada' ? (float)$l->total : (float)$l->total);
            }
        }
    ...
    public function finalizar(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $itensEstoque = DB::table('estoques as e')->join('produtos as p', 'p.id', '=', 'e.produto_id')
                ->where('e.empresa_id', $this->empresa_id)->whereIn('p.tipo_item', ['00', '01', '04'])
                ->select('e.produto_id', 'e.filial_id', 'e.quantidade', 'e.valor_compra')->get();

            foreach ($itensEstoque as $item) {
                DB::table('estoque_mensal_fechamentos')->insert([
                    'empresa_id' => $this->empresa_id, 'produto_id' => $item->produto_id, 'filial_id'  => $item->filial_id,
                    'quantidade' => $item->quantidade, 'valor_unitario_custo' => $item->valor_compra,
                    'mes' => $mes, 'ano' => $ano, 'created_at' => now(), 'updated_at' => now()
                ]);
            }

            DB::table('fechamentos_mensais')->insert([
                'empresa_id' => $this->empresa_id, 'mes' => $mes, 'ano' => $ano, 'regime' => $request->regime ?? 'competencia',
                'lucro_prejuizo_liquido' => $request->lucro_prejuizo_liquido ?? 0, 'status' => 'encerrado',
