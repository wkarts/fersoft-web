...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('estoque_fisico_movimentos')) {
            Schema::create('estoque_fisico_movimentos', function (Blueprint $table) {
                $table->unsignedBigInteger('id')->autoIncrement();
                $table->unsignedInteger('empresa_id');
                $table->unsignedInteger('filial_id')->nullable()->comment('NULL representa a Matriz');
                $table->unsignedInteger('usuario_id')->nullable();
                $table->unsignedInteger('produto_id');
                $table->unsignedBigInteger('pesagem_id')->nullable();
                $table->decimal('peso_bruto', 10, 2)->default(0)->comment('Peso Bruto do Ticket');
                $table->decimal('peso_impureza', 10, 2)->default(0)->comment('Peso deduzido de impurezas/descontos');
                $table->string('tipo', 20)->comment('entrada ou saida');
                $table->decimal('quantidade', 10, 2)->comment('Peso Líquido da Pesagem (KG)');
                $table->decimal('valor_unitario', 10, 4)->comment('Preço por KG negociado');
                $table->decimal('valor_total', 10, 2)->comment('Valor Total da Pesagem');
                $table->date('data_movimento')->comment('Data da finalização da pesagem');
                $table->timestamps();

                $table->index('empresa_id', 'estq_fisico_empresa_idx');
                $table->index('filial_id', 'estq_fisico_filial_idx');
                $table->index('usuario_id', 'estq_fisico_usuario_idx');
                $table->index('produto_id', 'estq_fisico_prod_id_idx');
                $table->index('pesagem_id', 'estq_fisico_pesagem_id_idx');
            });
        } else {
            $this->ensureBaseTenantColumns('estoque_fisico_movimentos');
            $this->ensureColumn('estoque_fisico_movimentos', 'produto_id', static function (Blueprint $table): void {
                $table->unsignedInteger('produto_id')->nullable();
            });
            $this->ensureColumn('estoque_fisico_movimentos', 'pesagem_id', static function (Blueprint $table): void {
                $table->unsignedBigInteger('pesagem_id')->nullable();
            });
            $this->ensureColumn('estoque_fisico_movimentos', 'peso_bruto', static function (Blueprint $table): void {
                $table->decimal('peso_bruto', 10, 2)->nullable()->default(0)->comment('Peso Bruto do Ticket');
            });
            $this->ensureColumn('estoque_fisico_movimentos', 'peso_impureza', static function (Blueprint $table): void {
                $table->decimal('peso_impureza', 10, 2)->nullable()->default(0)->comment('Peso deduzido de impurezas/descontos');
            });
            $this->ensureColumn('estoque_fisico_movimentos', 'tipo', static function (Blueprint $table): void {
                $table->string('tipo', 20)->nullable()->comment('entrada ou saida');
            });
            $this->ensureColumn('estoque_fisico_movimentos', 'quantidade', static function (Blueprint $table): void {
                $table->decimal('quantidade', 10, 2)->nullable()->comment('Peso Líquido da Pesagem (KG)');
            });
            $this->ensureColumn('estoque_fisico_movimentos', 'valor_unitario', static function (Blueprint $table): void {
                $table->decimal('valor_unitario', 10, 4)->nullable()->comment('Preço por KG negociado');
            });
            $this->ensureColumn('estoque_fisico_movimentos', 'valor_total', static function (Blueprint $table): void {
                $table->decimal('valor_total', 10, 2)->nullable()->comment('Valor Total da Pesagem');
            });
            $this->ensureColumn('estoque_fisico_movimentos', 'data_movimento', static function (Blueprint $table): void {
                $table->date('data_movimento')->nullable()->comment('Data da finalização da pesagem');
            });
            $this->ensureColumn('estoque_fisico_movimentos', 'created_at', static function (Blueprint $table): void {
                $table->timestamp('created_at')->nullable();
            });
            $this->ensureColumn('estoque_fisico_movimentos', 'updated_at', static function (Blueprint $table): void {
                $table->timestamp('updated_at')->nullable();
            });
        }

        $this->addIndexIfMissing('estoque_fisico_movimentos', 'estq_fisico_prod_id_idx', ['produto_id'], false);

        $this->addIndexIfMissing('estoque_fisico_movimentos', 'estq_fisico_pesagem_id_idx', ['pesagem_id'], false);

        $this->addIndexIfMissing('estoque_fisico_movimentos', 'estq_fisico_empresa_idx', ['empresa_id'], false);

        $this->addIndexIfMissing('estoque_fisico_movimentos', 'estq_fisico_filial_idx', ['filial_id'], false);

        $this->addIndexIfMissing('estoque_fisico_movimentos', 'estq_fisico_usuario_idx', ['usuario_id'], false);
        $this->addIndexIfMissing('estoque_fisico_movimentos', 'uq_estq_fisico_empresa_pesagem_produto', ['empresa_id', 'pesagem_id', 'produto_id'], true);

        $this->addForeignIfMissing('estoque_fisico_movimentos', 'fk_estq_fisico_empresa', 'empresa_id', 'empresas', 'id', 'cascade', null);

        $this->addForeignIfMissing('estoque_fisico_movimentos', 'fk_estq_fisico_pesagem', 'pesagem_id', 'pesagens', 'id', 'set null', null);

        $this->addForeignIfMissing('estoque_fisico_movimentos', 'fk_estq_fisico_produto', 'produto_id', 'produtos', 'id', 'cascade', null);

        $this->addForeignIfMissing('estoque_fisico_movimentos', 'estoque_fisico_filial_id_foreign', 'filial_id', 'filials', 'id', 'set null', null);

        $this->addForeignIfMissing('estoque_fisico_movimentos', 'estoque_fisico_usuario_id_foreign', 'usuario_id', 'usuarios', 'id', 'set null', null);
    }
