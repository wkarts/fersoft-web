    ...
    public function up()
    {
        if (!Schema::hasTable('tabela_preco_itens')) {
            Schema::create('tabela_preco_itens', function (Blueprint $table) {
                ...
                $table->string('tipo_frete', 50)->nullable()->comment('COLETA ou ENTREGA');
                $table->decimal('valor_kg', 15, 2)->default(0);
                $table->timestamps();
                $table->index('empresa_id', 'idx_tabela_preco_itens_empresa_id');
                $table->index('filial_id', 'idx_tabela_preco_itens_filial_id');
                $table->index('usuario_id', 'idx_tabela_preco_itens_usuario_id');
                $table->index('tabela_preco_id', 'idx_tabela_preco_itens_tabela_id');
                $table->index('produto_id', 'idx_tabela_preco_itens_produto_id');
            });
        } else {
            $this->ensureBaseTenantColumns('tabela_preco_itens');
        }

        $this->addForeignIfMissing('tabela_preco_itens', 'tabela_preco_itens_empresa_id_foreign', 'empresa_id', 'empresas', 'cascade');
        $this->addForeignIfMissing('tabela_preco_itens', 'tabela_preco_itens_filial_id_foreign', 'filial_id', 'filials', 'cascade');
        $this->addForeignIfMissing('tabela_preco_itens', 'tabela_preco_itens_usuario_id_foreign', 'usuario_id', 'usuarios', 'cascade');
    }

    public function down()
    {
        if (!Schema::hasTable('tabela_preco_itens')) { return; }
        $this->dropForeignIfExists('tabela_preco_itens', 'tabela_preco_itens_usuario_id_foreign');
        $this->dropForeignIfExists('tabela_preco_itens', 'tabela_preco_itens_filial_id_foreign');
        $this->dropForeignIfExists('tabela_preco_itens', 'tabela_preco_itens_empresa_id_foreign');
