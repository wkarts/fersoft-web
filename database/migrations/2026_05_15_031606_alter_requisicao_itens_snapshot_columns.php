...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('requisicao_itens')) { return; }

        $this->ensureBaseTenantColumns('requisicao_itens');

        Schema::table('requisicao_itens', function (Blueprint $table) {
    // reordenação/tipo executada abaixo por SQL bruto para manter compatibilidade.
        });

        if (!$this->isSqlite() && Schema::hasColumn('requisicao_itens', 'ca_snapshot')) {
            DB::statement("ALTER TABLE requisicao_itens MODIFY ca_snapshot VARCHAR(255) DEFAULT NULL AFTER motivo");
        }
        if (!$this->isSqlite() && Schema::hasColumn('requisicao_itens', 'fab_snapshot')) {
            DB::statement("ALTER TABLE requisicao_itens MODIFY fab_snapshot VARCHAR(255) DEFAULT NULL AFTER ca_snapshot");
        }
