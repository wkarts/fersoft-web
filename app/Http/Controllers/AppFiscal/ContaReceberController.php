...
class ContaReceberController extends Controller
{
	public function contas(Request $request){
		$contas_por_vendedor = $request->contas_por_vendedor;
		$usuario_id = $request->usuario_id;

		$clientesPluck = [];
		if($contas_por_vendedor){
			$usuario = Usuario::find($usuario_id);
			if($usuario != null){
				if($usuario->funcionario){
					$funcionario_id = $usuario->funcionario->id;
					if($funcionario_id){
						$clientesPluck = Cliente::where('funcionario_id', $funcionario_id)->pluck('id');
					}
				}
			}
		}
		...
		->where('status', 0)
		->when($contas_por_vendedor == true, function($q) use ($clientesPluck){

			return $q->whereIn('cliente_id', $clientesPluck);
		})
		->get();
