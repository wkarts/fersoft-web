...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('transferencias')) { return; }

        Schema::table('transferencias', function (Blueprint $table) {
            if (!Schema::hasColumn('transferencias', 'peso_bruto')) {
                $table->decimal('peso_bruto', 10, 3)->nullable()->default(0)->after('filial_entrada_id');
            }
            if (!Schema::hasColumn('transferencias', 'peso_liquido')) {
                $table->decimal('peso_liquido', 10, 3)->nullable()->default(0)->after('peso_bruto');
            }
        });
    }
    ...
    {
        if (!Schema::hasTable('transferencias')) { return; }
        $this->dropColumnIfExists('transferencias', 'peso_liquido');
        $this->dropColumnIfExists('transferencias', 'peso_bruto');
    }
