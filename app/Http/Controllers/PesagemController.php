        ...
        try {
            return DB::transaction(function () use ($request, $tipo) {
                if (in_array($tipo, ['cliente', 'fornecedor'], true)) {
                    ...
                    $data = $request->validate(['razao_social'=>'required|string|max:100','nome_fantasia'=>'nullable|string|max:80','telefone'=>'required|string|max:20','rua'=>'required|string|max:80','numero'=>'required|string|max:10','bairro'=>'required|string|max:50','cidade_id'=>'required|integer','cpf_cnpj'=>$exterior ? 'nullable|string|max:30' : 'required|digits_between:11,14','imagem'=>'nullable|image|mimes:jpg,jpeg,png,webp|max:2048']); unset($data['imagem']);
