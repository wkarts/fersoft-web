...
class UpdateViewVwNfeNumeracaoGaps extends Migration
{
    /**
     * Run the migrations.
     *
     * Versão CORRIGIDA:
     * - Ignora gaps já totalmente cobertos por inutilizações
     *   que possuam PROTOCOLO preenchido em nfe_inutilizacoes.
     */
    public function up()
    {
...
WHERE
    ...
    -- 🔹 Não exibir gaps cuja faixa esteja totalmente coberta
    -- por uma inutilização VÁLIDA (possui protocolo) na tabela nfe_inutilizacoes.
    AND NOT EXISTS (
        SELECT 1
        FROM nfe_inutilizacoes ni
        WHERE ni.empresa_id = ordenado.empresa_id
          AND (ni.filial_id <=> ordenado.filial_id) -- NULL-safe (Matriz)
...
SQL
     ...
     *
     * Versão ANTERIOR:
     * - Ignora gaps cobertos por inutilizações com status = 'autorizada'.
     * (É a versão que você tinha no up() da UpdateViewVwNfeNumeracaoGaps)
     */
...
WHERE
    ...
    -- 🔹 Versão anterior: considera inutilização apenas pelo status = 'autorizada'
    AND NOT EXISTS (
        SELECT 1
        FROM nfe_inutilizacoes ni
        WHERE ni.empresa_id = ordenado.empresa_id
