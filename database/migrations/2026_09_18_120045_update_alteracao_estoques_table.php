...
{
    private const REFERENCE_COLUMNS = ['empresa_id' => ['empresas', 'id', 'cascade', 'restrict'], 'produto_id' => ['produtos', 'id', 'cascade', 'restrict'], 'usuario_id' => ['usuarios', 'id', 'cascade', 'restrict']];
    private const MIGRATION_NAME = '2026_09_18_120045_update_alteracao_estoques_table';
    private const TABLE_NAME = 'alteracao_estoques';

    public function up()
    {
        $this->runWithSchemaLock(function () {
            // Valida o plano desta tabela antes do primeiro DDL; não preenche dados por suposição.
            $this->ensureTableExists('alteracao_estoques');
            $this->preflightColumn('alteracao_estoques', 'filial_id', ['column_type' => 'bigint unsigned', 'nullable' => true, 'column_default' => null, 'extra' => '', 'character_set_name' => null, 'collation_name' => null, 'column_comment' => null]);

            $this->ensureColumn('alteracao_estoques', 'filial_id', function (Blueprint $table) {
                return $table->unsignedBigInteger('filial_id')->nullable()->default(null);
