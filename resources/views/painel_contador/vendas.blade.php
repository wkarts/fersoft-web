	...
	<div class="card-body">
		...
		<form method="get" class="row">
			...
			<div class="col-lg-3">
				...
				<a class="btn btn-info" href="/contador/vendas">Limpar</a>
			</div>
		</form>
		<h4>Lista de Venda NFe</h4>
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{ $item->chave }}</td>
						<td>{{ $item->NfNumero }}</td>
						<td>{{ $item->data_emissao ? \Carbon\Carbon::parse($item->data_emissao)->format('d/m/Y H:i') : '--' }}</td>
						<td>
							@if($item->estado == 'APROVADO')
							<a title="Download XML" class="btn btn-sm btn-light" href="/contador/venda-download-xml/{{$item->id}}">
		...
		<form method="get" action="/contador/download-xml-nfe">
