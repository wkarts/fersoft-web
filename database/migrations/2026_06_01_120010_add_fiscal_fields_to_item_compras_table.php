...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('item_compras')) { return; }

        Schema::table('item_compras', function (Blueprint $table) {
            if (!Schema::hasColumn('item_compras', 'cst_icms')) { $table->string('cst_icms', 10)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'vbc_icms')) { $table->decimal('vbc_icms', 12, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'p_icms')) { $table->decimal('p_icms', 10, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'v_icms')) { $table->decimal('v_icms', 12, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'cst_ipi')) { $table->string('cst_ipi', 10)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'vbc_ipi')) { $table->decimal('vbc_ipi', 12, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'p_ipi')) { $table->decimal('p_ipi', 10, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'v_ipi')) { $table->decimal('v_ipi', 12, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'cst_pis')) { $table->string('cst_pis', 10)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'vbc_pis')) { $table->decimal('vbc_pis', 12, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'p_pis')) { $table->decimal('p_pis', 10, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'v_pis')) { $table->decimal('v_pis', 12, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'cst_cofins')) { $table->string('cst_cofins', 10)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'vbc_cofins')) { $table->decimal('vbc_cofins', 12, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'p_cofins')) { $table->decimal('p_cofins', 10, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'v_cofins')) { $table->decimal('v_cofins', 12, 4)->nullable(); }
            if (!Schema::hasColumn('item_compras', 'cst_is')) { $table->string('cst_is', 10)->nullable(); }
        });
    }
    ...
    public function down()
    {
        if (!Schema::hasTable('item_compras')) { return; }
        $columns = [
            'cst_is', 'v_cofins', 'p_cofins', 'vbc_cofins', 'cst_cofins',
            'v_pis', 'p_pis', 'vbc_pis', 'cst_pis',
            'v_ipi', 'p_ipi', 'vbc_ipi', 'cst_ipi',
            'v_icms', 'p_icms', 'vbc_icms', 'cst_icms',
        ];
        foreach ($columns as $column) {
            $this->dropColumnIfExists('item_compras', $column);
        }
    }
