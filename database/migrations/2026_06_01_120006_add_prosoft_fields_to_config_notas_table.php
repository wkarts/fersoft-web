...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('config_notas')) { return; }

        Schema::table('config_notas', function (Blueprint $table) {
            if (!Schema::hasColumn('config_notas', 'codigo_prosoft')) {
                $table->string('codigo_prosoft', 4)->nullable()->after('updated_at');
            }
            if (!Schema::hasColumn('config_notas', 'conta_estoque')) {
                $table->unsignedInteger('conta_estoque')->nullable()->after('codigo_prosoft');
            }
        });

        $this->addIndexIfMissing('config_notas', 'idx_config_notas_codigo_prosoft', ['codigo_prosoft']);
        $this->addIndexIfMissing('config_notas', 'idx_config_notas_conta_estoque', ['conta_estoque']);
    }

    public function down()
    {
        if (!Schema::hasTable('config_notas')) { return; }
        $this->dropIndexIfExists('config_notas', 'idx_config_notas_conta_estoque');
        $this->dropIndexIfExists('config_notas', 'idx_config_notas_codigo_prosoft');
        $this->dropColumnIfExists('config_notas', 'conta_estoque');
        $this->dropColumnIfExists('config_notas', 'codigo_prosoft');
    }
