...
class CreditoVendaController extends Controller
{
    ...
    public function somaVendas($clienteId, $filialId = null)
    {
        $query = CreditoVenda::where('cliente_id', $clienteId)
            ->where('empresa_id', $this->empresa_id)
            ->where('status', false);

        // se veio filial, filtra também
        if ($filialId !== null) {
        ...
        $total = $vendas->sum(function($v){
            return $v->venda->valor_total;
        });

        return response()->json($total);
    }

    public function somaVendas_($clienteId){
		$vendas = CreditoVenda::
