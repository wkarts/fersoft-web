		...
		({
			...
			success: function(e){
				...
				if(parsed && parsed.success === false){
					showFiscalError("Erro", parsed, "Falha ao transmitir CT-e")
					.then(() => {
						location.reload();
					});
				}
				...
				$('#btn-enviar').removeClass('spinner');
				$('#btn-enviar').removeClass('disabled');
				showFiscalError("Erro", e, "Falha ao transmitir CT-e")
...
function dacteTemp(){
	...
	if(cont > 1){
		...
		window.open(path+"cteSefaz/dacteTemp/"+id, "_blank");
	}
}
...
function gerarFatura(){
	...
	if(ids.length > 0){
		location.href = path+"cte/fatura/"+ids;
	}else{
		swal("Erro", "Selecione ao menos um documento para gerar a fatura!", "error")
	}
}
