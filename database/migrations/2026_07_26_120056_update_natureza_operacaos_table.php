...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('natureza_operacaos')) {
            return;
        }

        $this->ensureColumn('natureza_operacaos', 'cst_ibs_cbs', static function (Blueprint $table): void {
            $table->string('cst_ibs_cbs', 5)->nullable();
        });

        $this->ensureColumn('natureza_operacaos', 'class_trib_ibs_cbs', static function (Blueprint $table): void {
            $table->string('class_trib_ibs_cbs', 10)->nullable();
        });

        $this->ensureColumn('natureza_operacaos', 'perc_red_ibs', static function (Blueprint $table): void {
            $table->decimal('perc_red_ibs', 10, 4)->default(0);
        });

        $this->ensureColumn('natureza_operacaos', 'perc_red_cbs', static function (Blueprint $table): void {
            $table->decimal('perc_red_cbs', 10, 4)->default(0);
        });

        $this->applySafeColumnUpgrade([
            'table' => 'natureza_operacaos',
            'column' => 'categoria_conta_id',
            'type' => 'VARCHAR(3)',
            'nullable' => true,
