...
return new class extends Migration
{
    public function up()
    {
        ...
        $this->ensureBaseTenantColumns('conta_pagars');

        Schema::table('conta_pagars', function (Blueprint $table) {

            if (!Schema::hasColumn('conta_pagars', 'valor_csll')) { $table->decimal('valor_csll', 10, 2)->default(0)->after('valor_ir'); }
            if (!Schema::hasColumn('conta_pagars', 'retencoes_processadas')) { $table->boolean('retencoes_processadas')->default(false)->after('valor_csll'); }
            if (!Schema::hasColumn('conta_pagars', 'conta_id_origem')) { $table->integer('conta_id_origem')->nullable()->after('retencoes_processadas'); }
    ...
    public function down()
    {
        if (!Schema::hasTable('conta_pagars')) { return; }
        Schema::table('conta_pagars', function (Blueprint $table) {
                if (Schema::hasColumn('conta_pagars', 'conta_id_origem')) { $table->dropColumn('conta_id_origem'); }
                if (Schema::hasColumn('conta_pagars', 'retencoes_processadas')) { $table->dropColumn('retencoes_processadas'); }
                if (Schema::hasColumn('conta_pagars', 'valor_csll')) { $table->dropColumn('valor_csll'); }
        });
    }
