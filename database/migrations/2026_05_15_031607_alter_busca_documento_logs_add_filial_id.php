...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('busca_documento_logs')) { return; }

        $this->ensureBaseTenantColumns('busca_documento_logs');

        Schema::table('busca_documento_logs', function (Blueprint $table) {

            if (!Schema::hasColumn('busca_documento_logs', 'filial_id')) { $table->unsignedInteger('filial_id')->nullable()->after('empresa_id'); }

        });

        $this->addIndexIfMissing('busca_documento_logs', 'idx_busca_documento_logs_filial_id', ['filial_id']);
    }
    ...
    public function down()
    {
        if (!Schema::hasTable('busca_documento_logs')) { return; }
        Schema::table('busca_documento_logs', function (Blueprint $table) {
                if (Schema::hasColumn('busca_documento_logs', 'filial_id')) { $table->dropColumn('filial_id'); }
        });
    }
