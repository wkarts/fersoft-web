...
<head>
    <meta charset="UTF-8">
    <title>Recibo de Recebimento #{{ $conta->id }}</title>
    <style>
        @page { margin: 1.5cm; }
        * { box-sizing: border-box; }
        body { font-family: sans-serif; font-size: 13px; color: #333; margin: 0; padding: 0; }
        .recibo-box { width: 100%; border: 1px solid #000; padding: 25px; position: relative; min-height: 480px; }
        ...
        .dados-empresa { font-size: 10px; color: #555; margin-top: 3px; }
        .clear { clear: both; }
...
<div class="recibo-box">
    <div class="valor-box">R$ {{ number_format($conta->valor_pago ?? $conta->valor_integral, 2, ',', '.') }}</div>
    <div class="header-container">
        <div class="logo-area">
            @php
                $pathLogo = null;
                if($conta->filial && $conta->filial->logo) { $pathLogo = public_path('logos/' . $conta->filial->logo); }
                elseif($config->logo) { $pathLogo = public_path('logos/' . $config->logo); }
            @endphp
            @if($pathLogo && file_exists($pathLogo)) <img src="{{ $pathLogo }}" class="logo"> @endif
        </div>
        <div class="info-area">
            <div class="title">Recibo de Recebimento</div>
            <div class="empresa-nome">
                @if($conta->filial) {{ $conta->filial->razao_social ?? $conta->filial->descricao }}
                @else {{ $config->razao_social }} @endif
            </div>
    ...
    <div class="content">
        <p>Recebemos de <strong>{{ $conta->cliente->razao_social ?? $conta->cliente->nome }}</strong>, 
        a importância de <strong>R$ {{ number_format($conta->valor_pago ?? $conta->valor_integral, 2, ',', '.') }}</strong> 
        referente ao recebimento de: <strong>{{ $conta->referencia }}</strong>.</p>
        
        <p><strong>Data do Recebimento:</strong> {{ date('d/m/Y', strtotime($conta->data_pagamento)) }}<br>
        <strong>Forma de Recebimento:</strong> {{ $conta->tipo_pagamento }}</p>
    </div>
    <div class="footer">
        <div class="data-extenso">
            {{ $config->cidade ?? 'Salvador' }} - {{ $config->uf ?? 'BA' }}, {{ date('d') }} de 
            @php
        ...
        <div class="assinatura-container">
            <div class="linha-assinatura"></div><br>
