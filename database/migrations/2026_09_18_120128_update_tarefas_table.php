...
{
    private const REFERENCE_COLUMNS = ['empresa_id' => ['empresas', 'id', 'cascade', 'restrict'], 'filial_id' => ['filials', 'id', 'cascade', 'restrict'], 'funcionario_id' => ['funcionarios', 'id', 'cascade', 'restrict'], 'user_id' => ['usuarios', 'id', 'cascade', 'restrict'], 'usuario_id' => ['usuarios', 'id', 'cascade', 'restrict']];
    private const MIGRATION_NAME = '2026_09_18_120128_update_tarefas_table';
    private const TABLE_NAME = 'tarefas';

    public function up()
    {
        $this->runWithSchemaLock(function () {
            // Valida o plano desta tabela antes do primeiro DDL; não preenche dados por suposição.
            $this->ensureTableExists('tarefas');
            $this->preflightColumn('tarefas', 'frequencia', ['column_type' => 'varchar(50)', 'nullable' => true, 'column_default' => null, 'extra' => '', 'character_set_name' => 'utf8mb4', 'collation_name' => 'utf8mb4_unicode_ci', 'column_comment' => null]);
            $this->preflightIndex('tarefas', 'tarefas_user_id_foreign', ['user_id'], false);
            $this->preflightForeign('tarefas', 'tarefas_funcionario_id_foreign', 'funcionario_id', 'funcionarios', 'cascade', 'id', 'restrict');
            $this->preflightForeign('tarefas', 'tarefas_user_id_foreign', 'user_id', 'usuarios', 'cascade', 'id', 'restrict');

            $this->ensureColumn('tarefas', 'frequencia', function (Blueprint $table) {
                return $table->string('frequencia', 50)->nullable()->default(null)->charset('utf8mb4')->collation('utf8mb4_unicode_ci');
            ...
            $this->addIndexIfMissing('tarefas', 'tarefas_user_id_foreign', ['user_id'], false);
            $this->addForeignIfMissing('tarefas', 'tarefas_funcionario_id_foreign', 'funcionario_id', 'funcionarios', 'cascade', 'id', 'restrict');
            $this->addForeignIfMissing('tarefas', 'tarefas_user_id_foreign', 'user_id', 'usuarios', 'cascade', 'id', 'restrict');
        });
    }
