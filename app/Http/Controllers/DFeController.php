...
class DFeController extends Controller
{
    ...
    public function manifestar(Request $request){
        ...
        $configMatriz = ConfigNota::where('empresa_id', $this->empresa_id)->first();

        $isFilial = null;
        $cnpjDFe = preg_replace('/[^0-9]/', '', $configMatriz->cnpj);

        if($manifestaAnterior && $manifestaAnterior->filial_id) {
            $filial = Filial::find($manifestaAnterior->filial_id);
            if($filial) {
                $isFilial = $filial->id;
                $cnpjDFe = preg_replace('/[^0-9]/', '', $filial->cnpj ?? $filial->cpf_cnpj);
            }
        }
        ...
        $dfe_service = new DFeService([
            "atualizacao" => date('Y-m-d h:i:s'),
            "tpAmb" => 1,
            "razaosocial" => $configMatriz->razao_social,
            "siglaUF" => $configMatriz->UF,
            "cnpj" => $cnpjDFe,
            "schemes" => config('fiscal.default_schemes'),
            "versao" => "4.00",
            "tokenIBPT" => "AAAAAAA",
    ...
    public function download($chave){
        ...
        // SE O DFE FOI BAIXADO POR UMA FILIAL, USA OS DADOS DELA!
        if($dfe && $dfe->filial_id) {
            $filial = Filial::find($dfe->filial_id);
            if($filial) {
                $isFilial = $filial->id;
                $cnpjDFe = preg_replace('/[^0-9]/', '', $filial->cnpj ?? $filial->cpf_cnpj);
            }
        ...
        $dfe_service = new DFeService([
            "atualizacao" => date('Y-m-d h:i:s'),
            "tpAmb" => 1,
            "razaosocial" => $configMatriz->razao_social, // Mantém matriz para fallback de dados genéricos
            "siglaUF" => $configMatriz->UF,
            "cnpj" => $cnpjDFe, // CNPJ correto de quem solicitou
            "schemes" => config('fiscal.default_schemes'),
            "versao" => "4.00",
            "tokenIBPT" => "AAAAAAA",
            "CSC" => $configMatriz->csc,
            "CSCid" => $configMatriz->csc_id,
            "is_filial" => $isFilial // GARANTE O CERTIFICADO CORRETO!
        ], 55);
        try{
    ...
    private function getInfosDaNFe($xml){
        ...
        // Capturando as informações complementares da nota
        $infCpl = '';
    ...
    public function getDocumentosNovos(Request $request){
        try{
            ...
            if($local > 0){
                $filial = Filial::findOrFail($local);
                $isFilial = $local;
                $cnpjDFe = preg_replace('/[^0-9]/', '', $filial->cnpj ?? $filial->cpf_cnpj);
            }
            ...
            $dfe_service = new DFeService([
                ...
                "razaosocial" => $configMatriz->razao_social,
                "siglaUF" => $configMatriz->UF,
                "cnpj" => $cnpjDFe, // CNPJ correto de busca
                "schemes" => config('fiscal.default_schemes'),
                "versao" => "4.00",
                "tokenIBPT" => "AAAAAAA",
                "CSC" => $configMatriz->csc,
                "CSCid" => $configMatriz->csc_id,
                "is_filial" => $isFilial // Usa o certificado da filial!
            ], 55);
            ...
            if(!isset($docs['erro'])){
                ...
                foreach($docs as $d) {
                    ...
                    // TRAVA: Impede que a busca manual "resete" o estado da nota
                    if (!$notaExistente || $notaExistente->tipo == 0) {
                        if($d['valor'] > 0 && $d['nome']){
                            $d['filial_id'] = $local > 0 ? $local : null;

                            // 🟢 CAPTURA DA SITUAÇÃO DA SEFAZ (1=Autorizada, 2=Denegada, 3=Cancelada)
                            $situacao = 'AUTORIZADA';
                            if (isset($d['cSitNFe'])) {
                                if ($d['cSitNFe'] == '1') $situacao = 'AUTORIZADA';
                                elseif ($d['cSitNFe'] == '2') $situacao = 'DENEGADA';
                                elseif ($d['cSitNFe'] == '3') $situacao = 'CANCELADA';
                            }
                            $d['situacao_sefaz'] = $situacao;
    ...
    public function salvar(Request $request) {
        try {
            ...
            // --- INTEGRAÇÃO DO DE/PARA (PRODUTO FORNECEDOR) ---
            $produtoIds = $request->produtoId; // Array de IDs do sistema
            $codsProdXml = $request->codProdXml; // Array de códigos originais do XML

            if(isset($produtoIds) && count($produtoIds) > 0) {
                foreach($produtoIds as $key => $prodId) {
                    if(!empty($prodId)) {
                        // Verifica se este relacionamento do "De/Para" já existe para este fornecedor
                        $jaExisteVinculo = \App\Models\ProdutoFornecedor::where('fornecedor_id', $result->fornecedor_id)
                            ->where('codigo_fornecedor', $codsProdXml[$key])
                            ->where('produto_id', $prodId)
                            ->exists();

                        if(!$jaExisteVinculo) {
                            // Cria o vínculo de forma profissional para as próximas importações
                            \App\Models\ProdutoFornecedor::create([
                                'produto_id' => $prodId,
                                'fornecedor_id' => $result->fornecedor_id,
                                'codigo_fornecedor' => $codsProdXml[$key]
                            ]);
                        }
                    }
                }
            ...
            foreach($itens as $index => $i){
                ...
                $cst_icms_xml = ''; $vbc_icms = 0; $p_icms = 0; $v_icms = 0;
                $cst_ipi = null; $vbc_ipi = 0; $p_ipi = 0; $v_ipi = 0;
                $vbc_pis = 0; $p_pis = 0; $v_pis = 0;
                $vbc_cofins = 0; $p_cofins = 0; $v_cofins = 0;

                // ADICIONE ESTAS NOVAS VARIÁVEIS AQUI:
                $cst_ibs_cbs = null; $bc_ibs_cbs = 0; $aliq_ibs_uf = 0; $aliq_cbs = 0; $valor_ibs = 0; $valor_cbs = 0; $class_trib_ibs_cbs = null;

                if($xmlItem && isset($xmlItem->imposto)){
                    // ... Aqui dentro já tem a extração de ICMS, PIS, COFINS ...

                    // COLE ESTE BLOCO NO FINAL, AINDA DENTRO DO IF DO IMPOSTO:
                    if (isset($xmlItem->imposto->IBSCBS)) {
                        $ibscbs = $xmlItem->imposto->IBSCBS;
                        $cst_ibs_cbs = (string)($ibscbs->CST ?? '');
                        $class_trib_ibs_cbs = (string)($ibscbs->cClassTrib ?? '');
                        if (isset($ibscbs->gIBSCBS)) {
                            $bc_ibs_cbs = (float)($ibscbs->gIBSCBS->vBC ?? 0);
                            $aliq_ibs_uf = (float)($ibscbs->gIBSCBS->gIBSUF->pIBSUF ?? 0);
                ...
                ItemCompra::create([
                    ...
                    'valor_ibs'          => $valor_ibs,
                    'valor_cbs'          => $valor_cbs,
                    'class_trib_ibs_cbs' => $class_trib_ibs_cbs
                ]);
            ...
            if ($request->has('fatura_num')) {
                $nums = $request->fatura_num;
                $vencs = $request->fatura_venc;
                $vals = $request->fatura_val;
                $veiculos_parcelas = $request->fatura_veiculo;
                $formasPagamento = $request->forma_pagamento; // Captura a forma de pagamento

                $contaEmpresaId = $request->conta_empresa_id; // Captura a conta bancária escolhida

                foreach ($nums as $key => $num) {
                    $valorFinal = (float)str_replace(',', '.', str_replace('.', '', $vals[$key]));
                    $dataVencimento = \Carbon\Carbon::parse(str_replace("/", "-", $vencs[$key]))->format('Y-m-d');
                    $veiculoIdParcela = !empty($veiculos_parcelas[$key]) ? $veiculos_parcelas[$key] : $request->veiculo_id;

                    // INTELIGÊNCIA DA BAIXA AUTOMÁTICA
                    $formaPagamento = $formasPagamento[$key] ?? 'boleto';
                    $isAdiantamento = ($formaPagamento === 'adiantamento');

                    $statusPaga = ($isAdiantamento || ($contaEmpresaId != null && in_array($formaPagamento, ['dinheiro', 'pix', 'transferencia', 'cartao_combustivel']))) ? 1 : 0;
                    $dataPagamento = $statusPaga ? $result->data_emissao : null;
                    $valorPago = $statusPaga ? $valorFinal : 0;

                    $contaPagar = ContaPagar::create([
                        'compra_id' => $result->id,
                        'data_vencimento' => $dataVencimento,
                        'data_emissao' => $result->data_emissao,
                        'data_emissao_nfe' => $result->data_emissao,
                        'valor_integral' => $valorFinal,
                        'valor_original' => $valorFinal,
                        'valor_pago' => $valorPago,
                        'data_pagamento' => $dataPagamento,
                        'numero_nota_fiscal' => $result->nf,
                        'referencia' => "Parcela " . $num . " NF: " . $result->nf,
                        'categoria_id' => $request->categoria_conta_id ?? $request->categoria_id,
                        'status' => $statusPaga,
                        'tipo_pagamento' => $formaPagamento,
                        'conta_empresa_id' => $statusPaga ? $contaEmpresaId : null,
                        'empresa_id' => $this->empresa_id,
                        'fornecedor_id' => $result->fornecedor_id,
                        'veiculo_id' => $veiculoIdParcela,
                        'usuario_id' => get_id_user(),
                        'usuario_baixa_id' => $statusPaga ? get_id_user() : null,
                        'filial_id' => $result->filial_id
                    ]);

                    // EFETIVA AS BAIXAS NO SISTEMA
                    if($isAdiantamento) {
                        \App\Http\Controllers\AdiantamentoController::baixarAdiantamento($result->fornecedor_id, 'fornecedor', $valorFinal, $this->empresa_id, $contaPagar->id, get_id_user(), $result->filial_id);
                    }
                    elseif ($statusPaga && $contaEmpresaId) {
                        $contaBancaria = \DB::table('conta_empresas')->where('id', $contaEmpresaId)->first();
                        if ($contaBancaria) {
                            $saldoAtualizado = $contaBancaria->saldo - $valorFinal;

                            \DB::table('item_conta_empresas')->insert([
                                'conta_id' => $contaEmpresaId,
                                'descricao' => "Pagamento NF " . $result->nf,
                                'valor' => $valorFinal,
                                'tipo_pagamento' => $formaPagamento,
                                'tipo' => 'saida',
                                'data_pagamento' => $result->data_emissao,
                                'user_id' => get_id_user(),
                                'empresa_id' => $this->empresa_id,
                                'origem' => 'Conta Pagar',
    ...
    {
        // 1. Busca todos os manifestos pendentes (sem compra atrelada)
        $pendentes = ManifestaDfe::where('empresa_id', $this->empresa_id)
            ->where(function ($query) {
                $query->where('compra_id', 0)
                    ->orWhereNull('compra_id');
            })
            ->get();
        ...
        foreach ($pendentes as $dfe) {
            // 2. Verifica se a chave dessa nota já existe na tabela de Compras
            $compra = Compra::where('empresa_id', $this->empresa_id)
                ->where('chave', $dfe->chave)
                ->first();

            if ($compra) {
                // 3. Se a compra existe, atrela o ID da compra ao DFe
                $dfe->compra_id = $compra->id;

                // 4. Verifica se já existem parcelas geradas no Conta a Pagar para essa compra
                $temFatura = ContaPagar::where('empresa_id', $this->empresa_id)
        ...
        session()->flash('mensagem_sucesso', "Sincronização concluída! $atualizadas nota(s) pendente(s) foram atualizadas no DF-e.");
        return redirect()->back();
    }

    public function salvarProdutoAjax(Request $request) {
        try {
            // Cria o produto com TODOS os dados enviados pela modal
            $novoProduto = \App\Models\Produto::create([
                ...
                'ncm' => $request->ncm,
                'cest' => $request->cest,
                'cfop_interno' => $request->cfop, // Dependendo do seu banco, pode ser apenas 'cfop'
                'referencia' => $request->referencia,
                'quantidade' => $this->parseMoeda($request->quantidade ?? 1),
                'valor_compra' => $this->parseMoeda($request->valor_compra ?? 0),
                'percentual_lucro' => $this->parseMoeda($request->percentual_lucro ?? 0),
                'valor_venda' => $this->parseMoeda($request->valor_venda ?? 0),
                'unidade_compra' => $request->unidade_compra ?? 'UN',
                'unidade_venda' => $request->unidade_venda ?? 'UN',
                'codBarras' => ($request->codigo_barras == 'SEM GTIN' || empty($request->codigo_barras)) ? '' : $request->codigo_barras,
                'estoque_minimo' => $this->parseMoeda($request->estoque_minimo ?? 0),
                'categoria_id' => $request->categoria_id, // <-- O ID DA SUCATA ENTRA AQUI!
                'gerenciar_estoque' => $request->gerenciar_estoque ? 1 : 0,
                'inativo' => $request->inativo ? 1 : 0,

                // Derivado de Petróleo
                'anp' => $request->anp,
                'perc_glp' => $this->parseMoeda($request->p_glp ?? 0),
                'perc_gnn' => $this->parseMoeda($request->p_gnn ?? 0),
                'perc_gni' => $this->parseMoeda($request->p_gni ?? 0),
                'valor_partida' => $this->parseMoeda($request->valor_partida ?? 0),
                'unidade_tributavel' => $request->un_tributavel,
                'quantidade_tributavel' => $this->parseMoeda($request->qtd_tributavel ?? 0),

                // Dimensões e Peso
                'largura' => $this->parseMoeda($request->largura ?? 0),
                'altura' => $this->parseMoeda($request->altura ?? 0),
                'comprimento' => $this->parseMoeda($request->comprimento ?? 0),
                'peso_liquido' => $this->parseMoeda($request->peso_liquido ?? 0),
                'peso_bruto' => $this->parseMoeda($request->peso_bruto ?? 0),
                ...
                'codigo_enquadramento_ipi' => $request->codigo_enquadramento_ipi ?? '999',
                'perc_icms' => $this->parseMoeda($request->perc_icms ?? 0),
                'perc_pis' => $this->parseMoeda($request->perc_pis ?? 0),
                'perc_cofins' => $this->parseMoeda($request->perc_cofins ?? 0),
                'perc_ipi' => $this->parseMoeda($request->perc_ipi ?? 0),
                'perc_red_bc' => $this->parseMoeda($request->perc_red_bc ?? 0),
                'origem' => $request->origem ?? '0',
                'cst_pis_entrada' => $request->cst_pis_entrada,
                'cst_cofins_entrada' => $request->cst_cofins_entrada,
                'cst_ipi_entrada' => $request->cst_ipi_entrada,

                // (Adicione outras colunas de impostos se seu banco de dados pedir nomes diferentes)
            ]);
            ...
            // Retorna o erro exato para o console do navegador caso algo falhe (ex: coluna com nome diferente)
