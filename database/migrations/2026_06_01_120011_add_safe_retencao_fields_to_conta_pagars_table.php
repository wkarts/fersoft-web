...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('conta_pagars')) { return; }

        Schema::table('conta_pagars', function (Blueprint $table) {
            if (!Schema::hasColumn('conta_pagars', 'numero_nota_fiscal')) {
                $table->string('numero_nota_fiscal', 15)->nullable()->default('0');
            }
            if (!Schema::hasColumn('conta_pagars', 'valor_csll')) {
                $table->decimal('valor_csll', 10, 2)->nullable()->default(0);
            }
            if (!Schema::hasColumn('conta_pagars', 'retencoes_processadas')) {
                $table->boolean('retencoes_processadas')->nullable()->default(false);
            }
            if (!Schema::hasColumn('conta_pagars', 'conta_id_origem')) {
                $table->integer('conta_id_origem')->nullable();
            }
        });

        $this->addIndexIfMissing('conta_pagars', 'idx_conta_pagars_conta_id_origem', ['conta_id_origem']);
    }

    public function down()
    {
        if (!Schema::hasTable('conta_pagars')) { return; }
        $this->dropIndexIfExists('conta_pagars', 'idx_conta_pagars_conta_id_origem');
        $this->dropColumnIfExists('conta_pagars', 'conta_id_origem');
        $this->dropColumnIfExists('conta_pagars', 'retencoes_processadas');
        $this->dropColumnIfExists('conta_pagars', 'valor_csll');
        $this->dropColumnIfExists('conta_pagars', 'numero_nota_fiscal');
    }
