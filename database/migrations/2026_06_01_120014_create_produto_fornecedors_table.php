...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('produto_fornecedors')) {
            Schema::create('produto_fornecedors', function (Blueprint $table) {
                ...
                $table->string('descricao_fornecedor', 255)->nullable();
                $table->string('codigo_barras_fornecedor', 100)->nullable();
                $table->timestamps();

                $table->index('empresa_id', 'idx_produto_fornecedors_empresa_id');
                $table->index('filial_id', 'idx_produto_fornecedors_filial_id');
                $table->index('usuario_id', 'idx_produto_fornecedors_usuario_id');
                $table->index('produto_id', 'idx_produto_fornecedors_produto_id');
                $table->index('fornecedor_id', 'idx_produto_fornecedors_fornecedor_id');
                $table->index(['empresa_id', 'produto_id', 'fornecedor_id'], 'idx_produto_fornecedors_empresa_prod_for');
            });
        } else {
            $this->ensureBaseTenantColumns('produto_fornecedors');
        }

        $this->addForeignIfMissing('produto_fornecedors', 'produto_fornecedors_empresa_id_foreign', 'empresa_id', 'empresas', 'cascade');
        $this->addForeignIfMissing('produto_fornecedors', 'produto_fornecedors_filial_id_foreign', 'filial_id', 'filials', 'set null');
        $this->addForeignIfMissing('produto_fornecedors', 'produto_fornecedors_usuario_id_foreign', 'usuario_id', 'usuarios', 'set null');
    }

    public function down()
    {
        if (!Schema::hasTable('produto_fornecedors')) { return; }
        $this->dropForeignIfExists('produto_fornecedors', 'produto_fornecedors_usuario_id_foreign');
        $this->dropForeignIfExists('produto_fornecedors', 'produto_fornecedors_filial_id_foreign');
        $this->dropForeignIfExists('produto_fornecedors', 'produto_fornecedors_empresa_id_foreign');
