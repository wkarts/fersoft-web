    ...
    public function up()
    {
        if (!Schema::hasTable('bank_statement_transactions')) {
            Schema::create('bank_statement_transactions', function (Blueprint $table) {
                ...
                $table->string('tipo', 10)->comment('CREDITO ou DEBITO');
                $table->decimal('valor', 15, 2);
                $table->string('documento', 100)->nullable();
                $table->string('descricao', 255)->nullable();
                $table->string('status', 20)->default('pendente')->comment('pendente, conciliado, ignorado');
                $table->string('hash_transacao', 255)->nullable()->comment('Código único para evitar importar o mesmo OFX duas vezes');
                $table->string('import_hash', 64)->nullable();
                $table->timestamps();
                $table->index('empresa_id', 'idx_bank_statement_transactions_empresa_id');
                $table->index('filial_id', 'idx_bank_statement_transactions_filial_id');
                $table->index('usuario_id', 'idx_bank_statement_transactions_usuario_id');
                $table->index('conta_bancaria_id', 'idx_bank_stmt_conta_bancaria_id');
                $table->index('data_transacao', 'idx_bank_stmt_data_transacao');
                $table->index('hash_transacao', 'idx_bank_stmt_hash_transacao');
                $table->index('import_hash', 'idx_bank_stmt_import_hash');
            });
        } else {
            $this->ensureBaseTenantColumns('bank_statement_transactions');
        }

        $this->addForeignIfMissing('bank_statement_transactions', 'bank_statement_transactions_empresa_id_foreign', 'empresa_id', 'empresas', 'cascade');
        $this->addForeignIfMissing('bank_statement_transactions', 'bank_statement_transactions_filial_id_foreign', 'filial_id', 'filials', 'cascade');
        $this->addForeignIfMissing('bank_statement_transactions', 'bank_statement_transactions_usuario_id_foreign', 'usuario_id', 'usuarios', 'cascade');
    }

    public function down()
    {
        if (!Schema::hasTable('bank_statement_transactions')) { return; }
        $this->dropForeignIfExists('bank_statement_transactions', 'bank_statement_transactions_usuario_id_foreign');
        $this->dropForeignIfExists('bank_statement_transactions', 'bank_statement_transactions_filial_id_foreign');
        $this->dropForeignIfExists('bank_statement_transactions', 'bank_statement_transactions_empresa_id_foreign');
