...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('fornecedors')) { return; }

        $this->ensureBaseTenantColumns('fornecedors');

        Schema::table('fornecedors', function (Blueprint $table) {

            if (!Schema::hasColumn('fornecedors', 'tabela_preco_id')) { $table->unsignedBigInteger('tabela_preco_id')->nullable()->after('empresa_id'); }

        });

        $this->addIndexIfMissing('fornecedors', 'idx_fornecedors_tabela_preco_id', ['tabela_preco_id']);
    }
    ...
    public function down()
    {
        if (!Schema::hasTable('fornecedors')) { return; }
        Schema::table('fornecedors', function (Blueprint $table) {
                if (Schema::hasColumn('fornecedors', 'tabela_preco_id')) { $table->dropColumn('tabela_preco_id'); }
        });
    }
