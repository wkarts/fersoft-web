...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('item_compra_servicos')) {
            Schema::create('item_compra_servicos', function (Blueprint $table) {
                $table->unsignedBigInteger('id')->autoIncrement();
                $table->unsignedInteger('empresa_id');
                $table->unsignedInteger('filial_id')->nullable();
                $table->unsignedInteger('usuario_id')->nullable();
                $table->unsignedInteger('compra_id')->nullable();
                $table->string('codigo_servico_lc116', 10)->nullable();
                $table->text('discriminacao')->nullable();
                $table->decimal('valor_servico', 15, 2)->nullable()->default(0);
                $table->decimal('vbc_iss', 15, 2)->nullable()->default(0);
                $table->decimal('p_iss', 5, 2)->nullable()->default(0);
                $table->decimal('v_iss', 15, 2)->nullable()->default(0);
                $table->boolean('iss_retido')->default(false);
                $table->decimal('v_irrf', 15, 2)->nullable()->default(0);
                $table->decimal('v_inss', 15, 2)->nullable()->default(0);
                $table->decimal('v_csll', 15, 2)->nullable()->default(0);
                $table->decimal('v_pis', 15, 2)->nullable()->default(0);
                $table->decimal('v_cofins', 15, 2)->nullable()->default(0);
                $table->timestamps();

                $table->index('empresa_id', 'idx_item_compra_servicos_empresa_id');
                $table->index('filial_id', 'idx_item_compra_servicos_filial_id');
                $table->index('usuario_id', 'idx_item_compra_servicos_usuario_id');
                $table->index('compra_id', 'idx_item_compra_servicos_compra_id');
            });
        } else {
            $this->ensureBaseTenantColumns('item_compra_servicos');
        }

        $this->addForeignIfMissing('item_compra_servicos', 'item_compra_servicos_empresa_id_foreign', 'empresa_id', 'empresas', 'cascade');
        $this->addForeignIfMissing('item_compra_servicos', 'item_compra_servicos_filial_id_foreign', 'filial_id', 'filials', 'set null');
        $this->addForeignIfMissing('item_compra_servicos', 'item_compra_servicos_usuario_id_foreign', 'usuario_id', 'usuarios', 'set null');
    }

    public function down()
    {
        if (!Schema::hasTable('item_compra_servicos')) { return; }
        $this->dropForeignIfExists('item_compra_servicos', 'item_compra_servicos_usuario_id_foreign');
        $this->dropForeignIfExists('item_compra_servicos', 'item_compra_servicos_filial_id_foreign');
        $this->dropForeignIfExists('item_compra_servicos', 'item_compra_servicos_empresa_id_foreign');
        Schema::dropIfExists('item_compra_servicos');
    }
