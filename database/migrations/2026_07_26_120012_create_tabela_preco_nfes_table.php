...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('tabela_preco_nfes')) {
            Schema::create('tabela_preco_nfes', function (Blueprint $table) {
                $table->unsignedBigInteger('id')->autoIncrement();
                $table->unsignedInteger('empresa_id');
                $table->unsignedInteger('filial_id')->nullable();
                $table->unsignedInteger('usuario_id')->nullable();
                $table->unsignedInteger('produto_id');
                $table->decimal('preco_nfe', 15, 2)->default(0);
                $table->timestamps();

                $table->index('empresa_id', 'idx_tabela_preco_nfes_empresa_id');
                $table->index('filial_id', 'idx_tabela_preco_nfes_filial_id');
                $table->index('usuario_id', 'idx_tabela_preco_nfes_usuario_id');
                $table->index('produto_id', 'idx_tabela_preco_nfes_produto_id');
            });
        } else {
            $this->ensureBaseTenantColumns('tabela_preco_nfes');
            $this->ensureColumn('tabela_preco_nfes', 'produto_id', static function (Blueprint $table): void {
                $table->unsignedInteger('produto_id')->nullable();
            });
            $this->ensureColumn('tabela_preco_nfes', 'preco_nfe', static function (Blueprint $table): void {
                $table->decimal('preco_nfe', 15, 2)->nullable()->default(0);
            });
            $this->ensureColumn('tabela_preco_nfes', 'created_at', static function (Blueprint $table): void {
                $table->timestamp('created_at')->nullable();
            });
            $this->ensureColumn('tabela_preco_nfes', 'updated_at', static function (Blueprint $table): void {
                $table->timestamp('updated_at')->nullable();
            });
        }

        $this->addIndexIfMissing('tabela_preco_nfes', 'idx_tabela_preco_nfes_empresa_id', ['empresa_id'], false);

        $this->addIndexIfMissing('tabela_preco_nfes', 'idx_tabela_preco_nfes_produto_id', ['produto_id'], false);

        $this->addIndexIfMissing('tabela_preco_nfes', 'idx_tabela_preco_nfes_filial_id', ['filial_id'], false);

        $this->addIndexIfMissing('tabela_preco_nfes', 'idx_tabela_preco_nfes_usuario_id', ['usuario_id'], false);

        $this->addForeignIfMissing('tabela_preco_nfes', 'fk_preco_nfes_empresa', 'empresa_id', 'empresas', 'id', 'cascade', null);

        $this->addForeignIfMissing('tabela_preco_nfes', 'fk_preco_nfes_produto', 'produto_id', 'produtos', 'id', 'cascade', null);

        $this->addForeignIfMissing('tabela_preco_nfes', 'tabela_preco_nfes_filial_id_foreign', 'filial_id', 'filials', 'id', 'set null', null);

        $this->addForeignIfMissing('tabela_preco_nfes', 'tabela_preco_nfes_usuario_id_foreign', 'usuario_id', 'usuarios', 'id', 'set null', null);
    }
