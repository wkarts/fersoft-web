                ...
                <table class="table table-striped table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($record->prioridade == 'Urgente')
                                    <span class="badge bg-danger">🔴 Urgente</span>
                                @elseif($record->prioridade == 'Alta')
                                    <span class="badge bg-warning text-dark">🟡 Alta</span>
                                @elseif($record->prioridade == 'Baixa')
                                    <span class="badge bg-info text-white">🔵 Baixa</span>
                                @else
                            ...
                            <td>
                                @if($record->status == 'pendente' && $record->tempo_gasto_minutos > 0)
                                    <span class="badge bg-secondary text-white"><i class="fas fa-pause-circle"></i> Pausado</span>
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-sm" role="group">
                                    @if($record->status == 'pendente')
                                        <a href="/tarefas/iniciar/{{ $record->id }}" class="btn btn-info text-white" title="Iniciar Atividade">
                                    ...
                                    @elseif($record->status == 'em_andamento')
                                        ...
                                        <a href="/tarefas/pausar/{{ $record->id }}" class="btn btn-warning text-dark" title="Pausar Atividade">
                                            <i class="fas fa-pause"></i> Pausar
                                        </a>

                                        <a href="javascript:void(0)" onclick="finalizarTarefa({{ $record->id }}, {{ $isAtrasada ? 'true' : 'false' }})" class="btn btn-success text-white" title="Finalizar Atividade">
                                            <i class="fas fa-check-double"></i> Finalizar
                                        </a>
                                    @endif

                                    <a href="{{ $editUrl }}/{{ $record->id }}" class="btn btn-primary" title="Editar">
                                        <i class="fas fa-edit"></i> Editar
                                    </a>
                                    
                                    <a href="{{ $deleteUrl }}/{{ $record->id }}" class="btn btn-danger" onclick="return confirm('Tem certeza que deseja excluir esta tarefa?');" title="Excluir">
