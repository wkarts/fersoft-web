...
@section('content')
<style>
    .table-apuracao { table-layout: fixed; width: 100%; }
    .col-nome { width: 75%; }
    .val-col { width: 25%; text-align: right !important; padding-right: 2.5rem !important; }
            ...
            <table class="table table-apuracao">
                ...
                <tbody>
                    ...
                    @foreach($detalhes['receita_bruta'] as $det)
                        <tr class="row-detail rec"><td class="pl-15 text-muted small col-nome">{{$det->categoria_nome}}</td><td class="col-valor small">R$ {{number_format($det->total,2,',','.')}}</td></tr>
                    @endforeach

                    <tr class="row-group" onclick="toggleDet('cmv_det')">
                        <td class="pl-7 text-danger font-weight-bold col-nome"><i class="la la-plus-circle mr-2"></i> (-) CUSTO DE MERCADORIA (CMV)</td>
                        <td class="col-valor text-danger font-weight-bold">R$ {{number_format($cmvReal,2,',','.')}}</td>
                    </tr>
                    <tr class="row-detail cmv_det"><td class="pl-15 text-muted small col-nome">(+) Estoque Inicial</td><td class="col-valor small">R$ {{number_format($estoqueInicial,2,',','.')}}</td></tr>
                    <tr class="row-detail cmv_det"><td class="pl-15 text-muted small">(+) Compras Líquidas</td><td class="val-col small">R$ {{number_format($comprasDoMes,2,',','.')}}</td></tr>
                    <tr class="row-detail cmv_det"><td class="pl-15 text-muted small">(-) Estoque Final</td><td class="val-col small">R$ {{number_format($estoqueFinal,2,',','.')}}</td></tr>
                    ...
                    @foreach(['operacional' => 'OPERACIONAIS', 'administrativa' => 'ADMINISTRATIVAS', 'tributaria' => 'TRIBUTÁRIAS'] as $key => $label)
                        <tr class="row-group" onclick="toggleDet('{{$key}}')" style="background-color: #fff5f5">
                            <td class="pl-7 font-weight-bold text-danger col-nome"><i class="la la-plus mr-2"></i> (-) DESPESAS {{$label}}</td>
                            <td class="col-valor font-weight-bold text-danger">R$ {{number_format(abs($dados[$key]),2,',','.')}}</td>
                        </tr>
                        @foreach($detalhes[$key] as $det)
                            <tr class="row-detail {{$key}}"><td class="pl-15 text-muted small col-nome">{{$det->categoria_nome}}</td><td class="col-valor text-danger small">R$ {{number_format($det->total,2,',','.')}}</td></tr>
                        @endforeach
                    @endforeach
                    ...
                    @foreach(['financeira' => 'FINANCEIRO', 'nao_operacional' => 'NÃO OPERACIONAL'] as $key => $label)
                        <tr class="row-group bg-light" onclick="toggleDet('{{$key}}')">
                            <td class="pl-7 font-weight-bold col-nome"><i class="la la-plus mr-2"></i> (+/-) {{$label}}</td>
                            <td class="col-valor font-weight-bold {{ $dados[$key] < 0 ? 'text-danger' : 'text-success' }}">R$ {{number_format($dados[$key],2,',','.')}}</td>
                        </tr>
                        @foreach($detalhes[$key] as $det)
                            <tr class="row-detail {{$key}}"><td class="pl-15 text-muted small col-nome">{{$det->categoria_nome}}</td><td class="col-valor small">R$ {{number_format($det->total,2,',','.')}}</td></tr>
                        @endforeach
                    @endforeach
