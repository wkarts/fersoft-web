                                ...
                                <tr class="datatable-row">
                                    @foreach($fields as $field)
                                        @php
                                            ...
                                            if (in_array($field, ['data_manutencao', 'proxima_manutencao_data'])) {
                                                $value = optional($value)->format('d/m/Y');
                                            }

                                            if (in_array($field, ['quilometragem_atual', 'proxima_manutencao_km'])) {
                                                $value = $value !== null ? number_format($value, 0, ',', '.') . ' km' : null;
                                            }

                                            if ($field === 'custo') {
                                                $value = $value !== null ? 'R$ ' . number_format($value, 2, ',', '.') : 'R$ 0,00';
                                            }

                                            if ($field === 'veiculo.placa' && empty($value)) {
                                                $value = 'Veículo removido';
                                            }
                                        @endphp
