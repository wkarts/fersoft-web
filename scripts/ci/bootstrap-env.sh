...
# -----------------------------
# 3) Aplica secrets/geradas nas chaves (DEFINITIVO)
# -----------------------------
upsert_env "APP_KEY" "${APP_KEY}"
upsert_env "ENCRYPTION_KEY" "${ENCRYPTION_KEY}"

# -----------------------------
# 4) Overrides seguros pro CI (evita Reverb/Pusher no package:discover)
# -----------------------------
...
# -----------------------------
# 5) Se existirem secrets importantes, sobrescreve (sem exigir template)
# -----------------------------
# SMTP
...
# Reverb (secrets) - mesmo com broadcast log, deixo coerente se você quiser usar em algum job específico
