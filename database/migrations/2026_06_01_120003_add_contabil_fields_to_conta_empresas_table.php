...
return new class extends Migration
{
    public function up()
    {
        if (!Schema::hasTable('conta_empresas')) { return; }

        Schema::table('conta_empresas', function (Blueprint $table) {
            if (!Schema::hasColumn('conta_empresas', 'conta_contabil_id')) {
                $table->unsignedInteger('conta_contabil_id')->nullable()->after('filial_id');
            }
        });

        $this->addIndexIfMissing('conta_empresas', 'idx_conta_empresas_conta_contabil_id', ['conta_contabil_id']);
    }

    public function down()
    {
        if (!Schema::hasTable('conta_empresas')) { return; }
        $this->dropIndexIfExists('conta_empresas', 'idx_conta_empresas_conta_contabil_id');
        $this->dropColumnIfExists('conta_empresas', 'conta_contabil_id');
    }
