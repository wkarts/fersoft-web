...
return new class extends Migration
{
    public function up()
    {
        ...
        $this->ensureBaseTenantColumns('compras');

        Schema::table('compras', function (Blueprint $table) {
    // alteração de tipo executada abaixo por SQL bruto para manter compatibilidade.
        });

        if (!$this->isSqlite() && Schema::hasColumn('compras', 'xml_path')) {
            DB::statement("ALTER TABLE compras CHANGE COLUMN xml_path xml_path VARCHAR(60) NOT NULL");
        }
        if (!$this->isSqlite() && Schema::hasColumn('compras', 'chave')) {
            DB::statement("ALTER TABLE compras CHANGE COLUMN chave chave VARCHAR(50) NOT NULL");
        }
