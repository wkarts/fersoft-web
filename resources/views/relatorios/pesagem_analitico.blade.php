...
@section('content')
    ...
    <div class="cards">
        <div class="totais">
            ...
            <span class="stat-label">Peso Líquido Total</span>
        </div>
        <div class="totais">
            <span class="stat">{{ number_format($total_final, 2, ',', '.') }} kg</span>
            <span class="stat-label">Peso Final Líquido</span>
        </div>
        @if($configEmitente)
            <div class="totais">
    ...
    @forelse($relatorio as $item)
        @php
            // Consolidação exclusivamente visual: respeita a sequência cronológica real das leituras.
        ...
        <div class="card">
            ...
            <div class="grid">
                ...
                <div><small>Peso Inicial</small><br><strong>{{ $pesoInicialOperacao !== null ? number_format($pesoInicialOperacao, 2, ',', '.') . ' kg' : '-' }}</strong></div>
                <div><small>Peso Final</small><br><strong>{{ $pesoFinalVeiculo !== null ? number_format($pesoFinalVeiculo, 2, ',', '.') . ' kg' : '-' }}</strong></div>
                <div><small>Peso Líquido Total</small><br><strong>{{ number_format($item['peso_liquido'], 2, ',', '.') }} kg</strong></div>
                <div><small>Peso Final Líquido</small><br><strong>{{ number_format($item['peso_final'], 2, ',', '.') }} kg</strong></div>
