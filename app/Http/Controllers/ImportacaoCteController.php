    ...
    public function importarLote(Request $request)
    {
        ...
        foreach ($arquivos as $arquivo) {
            try {
                ...
                $xml->registerXPathNamespace('n', 'http://www.portalfiscal.inf.br/cte');
                $nodes = $xml->xpath('//n:infCte');
                ...
                if (ContaReceber::where('empresa_id', $empresa_id)->where('numero_nota_fiscal', $nCT)->where('nf_modelo', '57')->exists()) {
                    $duplicados++;
                    continue;
                }
                ...
                $natureza = NaturezaOperacao::where('empresa_id', $empresa_id)->where('natureza', 'LIKE', "%$natOpDesc%")->first() 
                            ?? NaturezaOperacao::where('empresa_id', $empresa_id)->first();
                
                if (!$natureza) throw new \Exception("Natureza de operação não encontrada.");

                $veiculo = Veiculo::where('empresa_id', $empresa_id)->first();
