...
return new class extends Migration
{
    public function up()
    {
        ...
        $this->ensureTableCollation('bank_statement_transactions', 'utf8mb4_general_ci');
        $this->applyColumnDefinition('bank_statement_transactions', 'empresa_id', 'INT UNSIGNED DEFAULT NULL');
        $this->applyColumnDefinition('bank_statement_transactions', 'filial_id', 'INT UNSIGNED DEFAULT NULL');
        $this->applyColumnDefinition('bank_statement_transactions', 'tipo', 'VARCHAR(10) NOT NULL COMMENT \'CREDITO ou DEBITO\'');
        $this->applyColumnDefinition('bank_statement_transactions', 'documento', 'VARCHAR(100) DEFAULT NULL');
        $this->applyColumnDefinition('bank_statement_transactions', 'descricao', 'VARCHAR(255) DEFAULT NULL');
        $this->applyColumnDefinition('bank_statement_transactions', 'status', 'VARCHAR(20) DEFAULT \'pendente\' COMMENT \'pendente, conciliado, ignorado\'');
        $this->applyColumnDefinition('bank_statement_transactions', 'hash_transacao', 'VARCHAR(255) DEFAULT NULL COMMENT \'Código único para evitar importar o mesmo OFX duas vezes\'');
        $this->applyColumnDefinition('bank_statement_transactions', 'import_hash', 'VARCHAR(64) DEFAULT NULL', 'updated_at', false);
    }
