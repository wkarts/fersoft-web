...
<style>
    ...
    /* Correções para o Select2 se alinhar ao Metronic */
    .select2-container {
        width: 100% !important;
    }
...
<div class="card card-custom gutter-b">
    <div class="card-header bg-primary py-3">
        <div class="card-title">
            <h3 class="card-label text-white font-weight-bolder">
                <i class="fa fa-link text-white mr-2"></i> Configurar De-Para de Resíduo MTR (SINIR / IEMA)
            </h3>
        </div>
    </div>
    ...
    <div class="card-body">
        ...
        <form action="{{ route('mtr.depara.store') }}" method="POST">
            @csrf
            ...
            <div class="border-box">
                ...
                <div class="row">
                        ...
                        <select name="sub_categoria_id" id="sub_categoria_id" class="form-control select2">
                            ...
                            @if(isset($subcategorias))
                                @foreach($subcategorias as $sub)
                                    <option value="{{ $sub->id }}" {{ old('sub_categoria_id') == $sub->id ? 'selected' : '' }}>
                        ...
                        <select name="produto_id" id="produto_id" class="form-control select2">
                            ...
                            @if(isset($produtos))
                                @foreach($produtos as $p)
                                    <option value="{{ $p->id }}" {{ old('produto_id') == $p->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-3 form-group">
                        ...
                        <input type="text" name="ncm" class="form-control" value="{{ old('ncm') }}" placeholder="Ex: 76020000">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 form-group">
                        ...
                        <div class="input-group">
                            <input type="text" name="cod_ibama" id="cod_ibama" class="form-control" value="{{ old('cod_ibama') }}" placeholder="Ex: 191202" required readonly>
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        <input type="text" name="descricao_residuo" id="descricao_residuo" class="form-control" value="{{ old('descricao_residuo') }}" placeholder="Ex: Metais ferrosos" required readonly>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-md-3 form-group">
                        ...
                        <select name="estado_fisico" class="form-control" required>
                            <option value="1" {{ old('estado_fisico', '1') == '4' ? 'selected' : '' }}>SÓLIDO</option>
                            <option value="2" {{ old('estado_fisico') == '2' ? 'selected' : '' }}>LÍQUIDO</option>
                            <option value="3" {{ old('estado_fisico') == '1' ? 'selected' : '' }}>SEMISSÓLIDO</option>
                            <option value="4" {{ old('estado_fisico') == '3' ? 'selected' : '' }}>GASOSO</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-3 form-group">
                        <label class="font-weight-bold">Acondicionamento (Embalagem): <span class="text-danger">*</span></label>
                        <select name="acondicionamento_id" class="form-control select2" required>
                            <option value="26" {{ old('acondicionamento_id') == '26' ? 'selected' : '' }}>BIG BAG</option>
                            <option value="4"  {{ old('acondicionamento_id') == '4' ? 'selected' : '' }}>CAÇAMBA ABERTA</option>
                            <option value="5"  {{ old('acondicionamento_id') == '5' ? 'selected' : '' }}>CAÇAMBA FECHADA</option>
                            <option value="21" {{ old('acondicionamento_id') == '21' ? 'selected' : '' }}>CAIXA</option>
                            <option value="22" {{ old('acondicionamento_id') == '22' ? 'selected' : '' }}>CAIXA DE PAPELÃO</option>
                            <option value="23" {{ old('acondicionamento_id') == '23' ? 'selected' : '' }}>CILINDRO</option>
                            <option value="3"  {{ old('acondicionamento_id') == '3' ? 'selected' : '' }}>CONTÊINER</option>
                            <option value="24" {{ old('acondicionamento_id') == '24' ? 'selected' : '' }}>FARDO</option>
                            <option value="8"  {{ old('acondicionamento_id') == '8' ? 'selected' : '' }}>GRANEL</option>
                            <option value="13" {{ old('acondicionamento_id') == '13' ? 'selected' : '' }}>OUTROS</option>
                            <option value="25" {{ old('acondicionamento_id') == '25' ? 'selected' : '' }}>PALETE</option>
                            <option value="2"  {{ old('acondicionamento_id') == '2' ? 'selected' : '' }}>SACO PLÁSTICO</option>
                            <option value="9"  {{ old('acondicionamento_id') == '9' ? 'selected' : '' }}>TAMBOR</option>
                        </select>
                    </div>

                    <div class="col-md-3 form-group">
                        <label class="font-weight-bold">Tecnologia de Tratamento: <span class="text-danger">*</span></label>
                        <select name="tratamento_id" class="form-control select2" required>
                            <option value="43" {{ old('tratamento_id') == '43' ? 'selected' : '' }}>Reciclagem</option>
                            <option value="60" {{ old('tratamento_id') == '60' ? 'selected' : '' }}>Coprocessamento</option>
                            <option value="4"  {{ old('tratamento_id') == '4' ? 'selected' : '' }}>Aterro Resíduos Classe I</option>
                            <option value="52" {{ old('tratamento_id') == '52' ? 'selected' : '' }}>Aterro Sanitário - RSU</option>
                            <option value="17" {{ old('tratamento_id') == '17' ? 'selected' : '' }}>Aterro Resíduos Classes IIA e IIB</option>
                            <option value="51" {{ old('tratamento_id') == '51' ? 'selected' : '' }}>Aterro de Reservaçaõ - RCC</option>
                            <option value="25" {{ old('tratamento_id') == '25' ? 'selected' : '' }}>Autoclave</option>
                            <option value="30" {{ old('tratamento_id') == '30' ? 'selected' : '' }}>Barragem de Rejeitos</option>
                            <option value="27" {{ old('tratamento_id') == '27' ? 'selected' : '' }}>Biodigestão</option>
                            <option value="31" {{ old('tratamento_id') == '31' ? 'selected' : '' }}>Biometanização</option>
                            <option value="32" {{ old('tratamento_id') == '32' ? 'selected' : '' }}>Biorremediação</option>
                            <option value="29" {{ old('tratamento_id') == '29' ? 'selected' : '' }}>Blendagem para Coprocessamento</option>
                            <option value="73" {{ old('tratamento_id') == '73' ? 'selected' : '' }}>Blendagem para Incineração</option>
                            <option value="50" {{ old('tratamento_id') == '50' ? 'selected' : '' }}>Compostagem</option>
                            <option value="33" {{ old('tratamento_id') == '33' ? 'selected' : '' }}>Desativação de Fosfina</option>
                            <option value="24" {{ old('tratamento_id') == '24' ? 'selected' : '' }}>Descontaminação de Lâmpadas</option>
                            <option value="28" {{ old('tratamento_id') == '28' ? 'selected' : '' }}>Desmontagem de Veículos</option>
                            <option value="34" {{ old('tratamento_id') == '34' ? 'selected' : '' }}>Disposição em Cava de Mineração</option>
                            <option value="36" {{ old('tratamento_id') == '36' ? 'selected' : '' }}>Estação de Transbordo - RSU</option>
                            <option value="72" {{ old('tratamento_id') == '72' ? 'selected' : '' }}>Fertirrigação</option>
                            <option value="38" {{ old('tratamento_id') == '38' ? 'selected' : '' }}>Fins Didáticos/Pesquisa</option>
                            <option value="39" {{ old('tratamento_id') == '39' ? 'selected' : '' }}>Gaseificação</option>
                            <option value="26" {{ old('tratamento_id') == '26' ? 'selected' : '' }}>Incineração</option>
                            <option value="40" {{ old('tratamento_id') == '40' ? 'selected' : '' }}>Microondas</option>
                            <option value="53" {{ old('tratamento_id') == '53' ? 'selected' : '' }}>Outros</option>
                            <option value="41" {{ old('tratamento_id') == '41' ? 'selected' : '' }}>Pilha de Estéril</option>
                            <option value="42" {{ old('tratamento_id') == '42' ? 'selected' : '' }}>Pirólise</option>
                            <option value="45" {{ old('tratamento_id') == '45' ? 'selected' : '' }}>Recuperação Energética</option>
                            <option value="44" {{ old('tratamento_id') == '44' ? 'selected' : '' }}>Rerrefino</option>
                            <option value="23" {{ old('tratamento_id') == '23' ? 'selected' : '' }}>Tratamento de Efluentes</option>
                            <option value="49" {{ old('tratamento_id') == '49' ? 'selected' : '' }}>Tratamento Térmico - outros</option>
                            <option value="46" {{ old('tratamento_id') == '46' ? 'selected' : '' }}>Triagem com Armazenamento</option>
                            <option value="61" {{ old('tratamento_id') == '61' ? 'selected' : '' }}>Triagem e Transbordo</option>
                            <option value="47" {{ old('tratamento_id') == '47' ? 'selected' : '' }}>Uso Agrícola</option>
                            <option value="48" {{ old('tratamento_id') == '48' ? 'selected' : '' }}>Uso Alimentação Animal</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-3 form-group">
                        ...
                        <select name="unidade_medida" class="form-control" required>
                            <option value="2"  {{ old('unidade_medida') == '2' ? 'selected' : '' }}>Quilograma (Kg)</option>
                            <option value="3"  {{ old('unidade_medida') == '3' ? 'selected' : '' }}>Tonelada (Ton)</option>
                            <option value="20" {{ old('unidade_medida') == '20' ? 'selected' : '' }}>Metro Cúbico (M³)</option>
                            <option value="21" {{ old('unidade_medida') == '21' ? 'selected' : '' }}>Litro (Lt)</option>
                        </select>
                    </div>

                    <div class="col-md-3 form-group">
                        <label class="font-weight-bold">Fator de Conversão:</label>
                        <input type="number" step="0.0001" name="fator_conversao" class="form-control" value="{{ old('fator_conversao', '1.0000') }}" required>
            ...
            <div class="d-flex justify-content-end mt-4">
                ...
                <button type="submit" class="btn btn-success font-weight-bold px-5">
                    <i class="fa fa-save"></i> Salvar Regra De-Para
                </button>
            </div>
        </form>
    </div>
...
<!-- Utilizamos yield javascript para obedecer ao layout Metronic da Fersoft -->
@section('javascript')
<script>
jQuery(document).ready(function($) {
    ...
    $('.btnSelecionarResiduo').on('click', function(e) {
        e.preventDefault();
        ...
        // Preenche os campos bloqueados da tela
        $('#cod_ibama').val(codigo);
        $('#descricao_residuo').val(descricao);

        // Fecha o modal de forma segura (previne o erro .modal is not a function)
        try {
            $('#modalIbama').modal('hide');
        } catch (err) {
            // Caso o Bootstrap conflite com o Metronic, fecha manualmente
