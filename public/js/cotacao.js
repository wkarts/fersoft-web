...
$('#addProd').click(() => {
	...
	if(codigo.length > 0 && nome.length > 0 && quantidade.length > 0 && parseFloat(quantidade.replace(',','.')) ) {
		...
		swal("Erro","Informe corretamente os campos para continuar!","warning")
...
function montaTabela(){
	...
	ITENS.map((v) => {
		...
		t += '<a href="#prod tbody" class="btn btn-sm btn-danger" onclick="deleteItem('+v.id+')">';
		t += '<i class="la la-trash"></i>'
		t += '</a>';
		t += '</span></td>';
...
$('#salvar-cotacao').click(() => {
	$('#salvar-cotacao').attr('disabled', 1)
	valida((msg) => {
		if(msg == ""){
			...
			$('#salvar-cotacao').removeAttr('disabled')
