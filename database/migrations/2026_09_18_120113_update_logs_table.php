...
{
    private const REFERENCE_COLUMNS = ['empresa_id' => ['empresas', 'id', 'cascade', 'restrict'], 'filial_id' => ['filials', 'id', 'set null', 'restrict'], 'usuario_id' => ['usuarios', 'id', 'cascade', 'restrict']];
    private const MIGRATION_NAME = '2026_09_18_120113_update_logs_table';
    ...
    public function up()
    {
        $this->runWithSchemaLock(function () {
            ...
            $this->ensureTableExists('logs');
            $this->preflightColumn('logs', 'registro_id', ['column_type' => 'bigint unsigned', 'nullable' => true, 'column_default' => null, 'extra' => '', 'character_set_name' => null, 'collation_name' => null, 'column_comment' => null]);
            $this->preflightIndex('logs', 'logs_registro_id_index', ['registro_id'], false);

            $this->ensureColumn('logs', 'registro_id', function (Blueprint $table) {
                return $table->unsignedBigInteger('registro_id')->nullable()->default(null);
            ...
            $this->addIndexIfMissing('logs', 'logs_registro_id_index', ['registro_id'], false);
        });
    }
