	...
	<div class="card-body">
		<div class="content d-flex flex-column flex-column-fluid" id="kt_content" >
			...
			<div class="col-lg-12" id="content">
				...
				@if($venda->chave != "")
				<h3 class="card-title">Chave: 
					<strong class="text-danger">{{$venda->chave}}</strong>
				</h3>
				@endif

				@if(empresaComFilial())
				<h3 class="card-title">Localização: 
					<strong class="text-danger">{{$venda->filial ? $venda->filial->descricao : 'Matriz'}}</strong>
				</h3>
				@endif

				@if($venda->troca)
				<h4 class="text-danger">Esta venda contém uma troca/devolução</h4>
				@if($venda->troca())
				<h4>Código da troca/devolução: <strong class="text-danger">{{$venda->troca()->id}}</strong> | Data: <strong class="text-danger">{{ \Carbon\Carbon::parse($venda->troca()->created_at)->format('d/m/Y H:i') }}</strong></h4>
				@endif
				<br>
				@endif
							...
							<div class="kt-section__body">
								...
								<div class="row">
									...
									<div class="col-lg-6 col-md-6 col-sm-6 col-12">
										...
										<a href="/vendas/edit_xml/{{$venda->id}}" class="btn btn-info">
											<i class="la la-file"></i>
											Alterar XML da venda
										</a>
										@endif
										@endif
									</div>
				...
				<div class="row col-12">
					...
					<div id="kt_datatable" class="datatable datatable-bordered datatable-head-custom datatable-default datatable-primary datatable-loaded">
						...
						<table class="datatable-table" style="max-width: 100%;" id="prod">
							<thead class="datatable-head">
								<tr class="datatable-row" style="left: 0px;">
				...
				<div class="row mt-2">
					<a target="_blank" href="/vendas/imprimirPedido/{{$venda->id}}" class="btn btn-light-success">
						<i class="la la-print"></i>
						Imprimir
					</a>

					@if($venda->estado == 'DISPONIVEL' || $venda->estado == 'REJEITADO')
					<a style="margin-left: 3px;" href="/vendas/edit/{{$venda->id}}" class="btn btn-light-warning">
					...
					<a style="margin-left: 3px;" href="/nf/imprimir/{{$venda->id}}" class="btn btn-light-info">
						<i class="la la-print"></i>
						Imprimir Danfe
					</a>
					<a style="margin-left: 3px;" href="/nf/imprimirSimples/{{$venda->id}}" class="btn btn-light-primary">
						<i class="la la-print"></i>
